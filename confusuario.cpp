...
void confUsuario::fbuscaDiretoriosAnimes() {
  ...
  for (int i = 0; i < vdiretorioAnimes.size(); i++) {
    ...
    while (lit.hasNext()) {
      ...
      // Usamos isso para pegar o número do episódio e o nome do anime a partir
      // do nome do arquivo
      QString lfileName =
      ...
      for (int w = 0; w < vlistaAnimes.size(); w++) {
        ...
        // função vai comparar os nomes certos Compara os nomes, sempre ignorando
        // letras maiusculas e minusculas para garantir que o anime será
        // encontrado.
        ...
        if (formatador.fcomparaNomes(lfileName, vlistaAnimes[w]->vnome)) {
          ...
          vlistaAnimes.remove(w);
          break;
        } else if (formatador.fcomparaNomes(lfileName,
                                            vlistaAnimes[w]->vnomeIngles)) {
          vdiretorioEspecificoAnime.insert(vlistaAnimes[w]->vid,
                                           lfile.fileName());
          vlistaAnimes.remove(w);
          break;
          ...
          if (!vlistaAnimes[w]->vnomeAlternativo.isEmpty()) {
            for (int z = 0; z < vlistaAnimes[w]->vnomeAlternativo.size(); z++) {
              if (formatador.fcomparaNomes(
                      lfileName, vlistaAnimes[w]->vnomeAlternativo.at(z))) {
                vdiretorioEspecificoAnime.insert(vlistaAnimes[w]->vid,
                                                 lfile.fileName());
                vlistaAnimes.remove(w);
                break;
...
void confUsuario::fselecionaPastaEspecificaAnime(const QString &ridAnime,
                                                 const QString &rdirAnime) {
