...
MainWindow::MainWindow(QWidget *parent)
  ...
  fatualizaFontes();
  vtituloNormalMedio = "background: transparent; font: 75 7pt \"Calibri\"; "
                       "font-weight: bold; color: rgb(20, 20, 20);";
  vtituloNormalPequeno = "background: transparent; font: 75 6pt \"Calibri\"; "
                         "font-weight: bold; color: rgb(20, 20, 20);";
  vfonteNormal = "background: transparent; font: 75 8pt \"Calibri\"; "
                 "font-weight: bold; color: rgb(20, 20, 20);";
  vfonteNormalNota = "background: transparent; font: 75 8pt \"Calibri\"; "
                     "font-weight: bold; color: rgb(53, 68, 120);";
  vfonteNormalAnimeEncontrado =
      "background: transparent; font: 75 8pt \"Calibri\"; font-weight: bold; "
      "color: rgb(53, 120, 56);";
  vfonteNormalAnimeNaoComecou =
      "background: transparent; font: 75 8pt \"Calibri\"; font-weight: bold; "
      "color: rgb(120, 60, 53);";
  vfonteTitulo = "background: transparent;font: 75 " + fontSizeTitulo + "pt " +
                 fontName + ";font-weight: bold; color: rgb(20, 20, 20);";
  vfonteSubtitulo = "background: transparent;font: 75 " +
                    fontSizeTituloAlternativo + "pt " + fontName +
                    "; color: rgb(20, 20, 20);";
  vfonteStatus = "background: transparent;font: 75 " + fontSizeStatus + "pt " +
                 fontName + "; color: rgb(20, 20, 20);";
  vfonteInfo = "background: transparent;font: 75 " + fontSizeInfo + "pt " +
               fontName + ";font-weight: bold; color: rgb(20, 20, 20);";
  vfonteMensagem = "background: transparent;font: 75 " + fontSizeMensagem +
                   "pt " + fontName + ";color: rgb(20, 20, 20);";
  vfonteMensagemAnime = "background: transparent;font: 75 " +
                        fontSizeMensagemAnime + "pt " + fontName +
                        ";color: rgb(20, 20, 20);";
  ...
  canilist->frecebeAutorizacao(jconfig.fretornaUsuario(),
                               jconfig.fretornaCodigoAutorizacao());
  ...
  connect(timer, &QTimer::timeout, this,
          QOverload<>::of(&MainWindow::on_botaoRefresh_clicked));
  // O segundo timer é uma contagem de segundo em segundo pra mostrar quanto
  // tempo falta para o timer de cima chegar à 0
  timerRefresh = new QTimer(this);
  connect(timerRefresh, &QTimer::timeout, this,
          QOverload<>::of(&MainWindow::fatualizaRefreshTimer));
  tryTimer = new QTimer(this);
  tryTimer->start(30000);
  connect(tryTimer, &QTimer::timeout, this,
          QOverload<>::of(&MainWindow::fatualizaAnilist));
  timerChecaAnimes = new QTimer(this);
  timerChecaAnimes->start(10000);
  connect(timerChecaAnimes, &QTimer::timeout, this,
          QOverload<>::of(&MainWindow::fVerificaAnimeAberto));
  timerChecaDownloadPorAno = new QTimer(this);
  connect(timerChecaDownloadPorAno, &QTimer::timeout, this,
          QOverload<>::of(&MainWindow::fsetDownloadImagensAnimesPorAno));
  ...
  connect(canilist, &anilist::sterminouDownload, this,
          &MainWindow::fcarregouListaTeste, Qt::QueuedConnection);
  connect(&jconfig, &janeladeconfig::sauthcodesave, this,
          &MainWindow::fretryAnilist);
  connect(&jconfig, &janeladeconfig::ssavebutton, this,
          &MainWindow::fretryAnilist);
  connect(&jconfig, &janeladeconfig::ssavebutton, this,
          &MainWindow::fgetConfigurations);
  connect(&jtorrent, &janelatorrent::infoAnime, this,
          &MainWindow::fInfoAnimeTorrent, Qt::QueuedConnection);

  // Quando baixa todas as imagens, carrega as imagens na página
  connect(cfiledownloader, &filedownloader::sterminouLista, this,
          &MainWindow::fcarregaImagensBackground, Qt::QueuedConnection);
  connect(cfiledownloader, &filedownloader::sid, this,
          &MainWindow::fsetIdBaixado, Qt::QueuedConnection);
  connect(cleitorListaAnimes, &leitorlistaanimes::sAnimeAdicionadoNaLista, this,
          &MainWindow::fsetIdAdicionado, Qt::QueuedConnection);
  connect(cconfUsuario, &confUsuario::schecouPastas, this,
          &MainWindow::fmandaDiretoriosArquivos, Qt::QueuedConnection);
  connect(this, &MainWindow::sterminouCarregarImagens, this,
          &MainWindow::fcarregaImagensLista, Qt::QueuedConnection);
  connect(this, &MainWindow::sanimeReconhecido, this,
          &MainWindow::fMostraAnimeAberto, Qt::QueuedConnection);

  // Se a lista foi carregada com sucesso, temos certeza de que o usuário já foi
  // lido.
  this->setWindowTitle("Atom - " + jconfig.fretornaUsuario());
  ui->labelUsername->setText(jconfig.fretornaUsuario());
  // Caso tenha lido a lista com sucesso, começa a procurar pelos animes no
  // computador.
  ...
  //    connect(cfiledownloader, &filedownloader::savatar, this,
  //    &MainWindow::fcarregaImagensLista, Qt::QueuedConnection);
  ...
  // Caso haja algum erro onde não foi possível atualizar o anilist, as ações
  // não atualizadas são salvas em um bloco de texto. Ao iniciar o programa, é
  // verificado se existe alguma atualização que não foi completada ainda
  ...
  if (!vlistaAcoes.isEmpty()) {
    ...
    if (lfile.open(QIODevice::WriteOnly | QIODevice::Append)) {
      ...
      foreach (QStringList key, vlistaAcoes.keys()) {
        lstreamTexto << key.at(0) << "-" << key.at(1) << "-" << vlistaAcoes[key]
                     << endl;
...
void MainWindow::fcarregouListaSucesso(bool ldownload) {
  // Ele sempre vai tentar baixar a lista. Caso tenha baixado, isso vira true e
  // não é necessário baixar de novo Caso não tenha baixado a lista ainda ou deu
  // algum erro no download, tenta baixar a lista de novo.
  if (!ldownload)
    fcarregouListaFalha();
  else {
  ...
  // Posso por o recebe configs em uma função própria com sinal de qunado salvar
  // as configs
  ...
  ui->NumPagina->setText(
      "Watching - " + QString::number(vlistaSelecionada.size()) +
...
void MainWindow::fcarregouListaFalha() {
  ...
  ui->labelMensagem->setText(
      "Trying to download the anime lists. The application may freeze "
...
void MainWindow::fsetIdBaixado(const QString &lid) {
  ...
  ui->labelLoading->setText(
      "Downloaded " + QString::number(vprogressoLoadingBar) + " of " +
...
void MainWindow::fVerificaAnimeAberto() {
  // Se eu fizer um ponteiro disso, o ponteiro vai poder ser deletado sem fechar
  // o programa na memória?
  ...
  foreach (const auto &janela, vlistaJanelas) {
    ...
    if (!QString::fromStdString(janela.GetTitle()).isEmpty()) {
      ...
      foreach (QString player, vPlayers) {
        if (vrefreshAcontecendo)
          return;
        if (nomejanela.contains("online in high quality - ") &&
            player.compare("kissanime", Qt::CaseInsensitive) == 0)
          nomejanela.replace("online in high quality",
                             "kissanime"); // Sim, é uma gambiarra.
        else if (nomejanela.contains(".mkv", Qt::CaseInsensitive) ||
                 nomejanela.contains(".mp4", Qt::CaseInsensitive)) {
          if (player.compare("Media Player Classic", Qt::CaseInsensitive) == 0)
            nomejanela.append(" - Media Player Classic");
          else if (player.compare("WebTorrent Desktop", Qt::CaseInsensitive) ==
                   0)
        ...
        if (nomejanela.contains(player, Qt::CaseInsensitive)) {
          ...
          QString vidAnime =
              cleitorListaAnimes->fprocuraAnimeNasListas(nomeAnime);
          if (!vidAnime.isEmpty()) {
            ...
            ui->labelAnimeAssistindoEpisodio->setText(
                "Episode not recognized.");
            ui->labelMensagemAnimeAssistindo->setText(
                "If the anime wasn't recognized correctly, search for the right"
      ...
      ui->labelFundoAnimeAssistindoTitulo->setStyleSheet(
          "background: transparent");
      ui->labelFundoAnimeAssistindoEpisodio->setStyleSheet(
          "background: transparent");
...
void MainWindow::fMostraAnimeAberto(QString ridAnime, const QString &rnomeAnime,
                                    const QString &repisodioAnime) {
  ...
  // Vai checar a cada 10 segundos, então tem que assistir dois minutos de anime
  // pra contar. Assim, atualiza os curtas ao mesmo tempo que não atualiza se
  // você mal assistir
...
/// ASSIM QUE O DESIGN ESTIVER PRONTO, CHECAR TAMBÉM SE A SINOPSE É MAIOR QUE O
/// LABEL E AJEITAR ISSO
void MainWindow::finfoAnimeSelecionado() {
  //    fVerificaAnimeAberto();
  if (!vlistaSelecionada.isEmpty() || vanimeSelecionado != -1) {
    ui->labelInfoNomeAnime->setText(
        vlistaSelecionada[vanimeSelecionado]->vnome);
    ui->labelInfoNomeAnimeIngles->setText(
        vlistaSelecionada[vanimeSelecionado]->vnomeIngles);
    ui->labelInfoSinopse->setText(
        vlistaSelecionada[vanimeSelecionado]->vsinopse);
    ...
    ui->labelInfoMediaSite->setText(QString::number(
        vlistaSelecionada[vanimeSelecionado]->vnotaMediaSite.toDouble() / 10,
        'f', 2));
    ui->labelInfoNota->setText(
        vlistaSelecionada[vanimeSelecionado]->vnotaMediaPessoal);
    ui->labelInfoRelease->setText(
        vlistaSelecionada[vanimeSelecionado]->vdataEpisodio);
    if (vlistaSelecionada[vanimeSelecionado]->vnumProximoEpisodioLancado ==
            "" &&
        vlistaSelecionada[vanimeSelecionado]->vstatus != "Finished Airing")
      ui->labelInfoEpisodiosLancados->setText("-");
    else if (vlistaSelecionada[vanimeSelecionado]
                 ->vnumProximoEpisodioLancado.toInt() == 0)
      ui->labelInfoEpisodiosLancados->setText("All Episodes");
    else
      ui->labelInfoEpisodiosLancados->setText(
          QString::number(vlistaSelecionada[vanimeSelecionado]
                              ->vnumProximoEpisodioLancado.toInt() -
                          1) +
          " Episodes");
    ui->labelInfoTipo->setText(vlistaSelecionada[vanimeSelecionado]->vformato);
    ui->labelInfoNota2->setText(
        vlistaSelecionada[vanimeSelecionado]->vnotaMediaPessoal);
    ui->labelInfoProgresso->setText(
...
void MainWindow::fcarregaImagensLista() {
  ...
  if (!vlistaSelecionada.isEmpty()) {
    ...
    if (pix.load(cconfBase->vdiretorioImagensGrandes +
                     vlistaSelecionada[vanimeSelecionado]->vid + ".jpg",
                 "jpg")) {
      ui->imagemAnimeGrande->setPixmap(pix);
    }
    // Se não conseguir carregar como jpg, tenta carregar como png
    else if (pix.load(cconfBase->vdiretorioImagensGrandes +
                          vlistaSelecionada[vanimeSelecionado]->vid + ".png",
                      "png")) {
      ui->imagemAnimeGrande->setPixmap(pix);
    }
    // Se a imagem grande não existir, vai carregar a imagem média no lugar
    else if (pix.load(cconfBase->vdiretorioImagensMedio +
                          vlistaSelecionada[vanimeSelecionado]->vid + ".jpg",
                      "jpg")) {
      ui->imagemAnimeGrande->setPixmap(pix);
    } else if (pix.load(cconfBase->vdiretorioImagensMedio +
    ...
    ui->labelFundoAnime00Progresso->setStyleSheet(
        "background: rgb(185,201,250);");
    ui->labelFundoAnime00Titulo->setStyleSheet("background: rgb(185,201,250);");
    ui->labelAnime00Titulo->setText(
        vlistaSelecionada[0 + (12 * (vpagina - 1))]->vnome);
    ...
    if (!carquivos
             ->fprocuraEpisodio(vlistaSelecionada[0 + (12 * (vpagina - 1))])
             .isEmpty() &&
        vtipoAtual == "anime")
      ui->labelAnime00Progresso->setStyleSheet(vfonteNormalAnimeEncontrado);
    else if (vlistaSelecionada[0 + (12 * (vpagina - 1))]->vstatus.compare(
                 "Not Aired Yet", Qt::CaseInsensitive) == 0)
      ui->labelAnime00Progresso->setStyleSheet(vfonteNormalAnimeNaoComecou);
    else
      ui->labelAnime00Progresso->setStyleSheet(vfonteNormal);
    ui->labelAnime00Progresso->setText(
        "Progresso: " +
        vlistaSelecionada[0 + (12 * (vpagina - 1))]->vnumEpisodiosAssistidos +
        "/" + vlistaSelecionada[0 + (12 * (vpagina - 1))]->vnumEpisodiosTotais);
    ui->labelAnime00Nota->setAlignment(Qt::AlignCenter);
    ui->labelAnime00Nota->setText(
        "Nota: " +
        vlistaSelecionada[0 + (12 * (vpagina - 1))]->vnotaMediaPessoal + "/10");
    if (vlistaAtual == "busca" || vlistaAtual.contains("season")) {
      ui->labelFundoAnime00Lista->setStyleSheet(
          "background: rgb(185, 202, 250); font: 75 8pt \"Calibri\"; "
          "font-weight: bold; color: rgb(20, 20, 20);");
      bool ok;
      // Checa se a lista é um número válido
      vlistaSelecionada[(12 * (vpagina - 1))]->vlista.toInt(&ok);
      if (ok)
        ui->labelFundoAnime00Lista->setText(
            vlistaSelecionada[(12 * (vpagina - 1))]->vSeasonSemAno);
      else
        ui->labelFundoAnime00Lista->setText(
            vlistaSelecionada[0 + (12 * (vpagina - 1))]->vlista);
    } else {
    ...
    if (vimagemCarregada[vlistaSelecionada[0 + (12 * (vpagina - 1))]->vid]) {
      if (pix.load(cconfBase->vdiretorioImagensMedio +
                       vlistaSelecionada[0 + (12 * (vpagina - 1))]->vid +
                       ".jpg",
                   "jpg")) {
    ...
    // PRO LOOP
    // remove imagemAnime remove labelAnime remove labelFundoAnime remove nota
    // remove progresso remove titulo Ai pega o valor que sobrar e joga na
    // condição if valor == 0 && vlistaSelecionada.size() != 0 if valor != 0 &&
    // vlistaSelecionada.size() > VALOR+(12*(vpagina-1))
  ...
  if (vlistaSelecionada.size() > 1 + (12 * (vpagina - 1))) {
    ...
    ui->labelFundoAnime01Progresso->setStyleSheet(
        "background: rgb(185,201,250);");
    ...
    ui->labelAnime01Titulo->setText(
        vlistaSelecionada[1 + (12 * (vpagina - 1))]->vnome);
    ui->labelAnime01Titulo->setAlignment(Qt::AlignCenter);
    ui->labelAnime01Titulo->setWordWrap(true);
    if (!carquivos
             ->fprocuraEpisodio(vlistaSelecionada[1 + (12 * (vpagina - 1))])
             .isEmpty() &&
        vtipoAtual == "anime")
      ui->labelAnime01Progresso->setStyleSheet(vfonteNormalAnimeEncontrado);
    else if (vlistaSelecionada[1 + (12 * (vpagina - 1))]->vstatus.compare(
                 "Not Aired Yet", Qt::CaseInsensitive) == 0)
    ...
    ui->labelAnime01Progresso->setText(
        "Progresso: " +
        vlistaSelecionada[1 + (12 * (vpagina - 1))]->vnumEpisodiosAssistidos +
    ...
    ui->labelAnime01Nota->setText(
        "Nota: " +
        vlistaSelecionada[1 + (12 * (vpagina - 1))]->vnotaMediaPessoal + "/10");
    if (vlistaAtual == "busca" || vlistaAtual.contains("season")) {
      ui->labelFundoAnime01Lista->setStyleSheet(
          "background: rgb(185, 202, 250); font: 75 8pt \"Calibri\"; "
          "font-weight: bold; color: rgb(20, 20, 20);");
      bool ok;
      // Checa se a lista é um número válido
      ...
      if (ok)
        ui->labelFundoAnime01Lista->setText(
            vlistaSelecionada[1 + (12 * (vpagina - 1))]->vSeasonSemAno);
      else
        ui->labelFundoAnime01Lista->setText(
            vlistaSelecionada[1 + (12 * (vpagina - 1))]->vlista);
    } else {
    ...
    if (vimagemCarregada[vlistaSelecionada[1 + (12 * (vpagina - 1))]->vid]) {
      if (pix.load(cconfBase->vdiretorioImagensMedio +
                       vlistaSelecionada[1 + (12 * (vpagina - 1))]->vid +
                       ".jpg",
                   "jpg")) {
        ...
        ui->imagemAnime01->clear(); // Trocar isso por imagem coringa pro caso
                                    // de não ter imagem do anime
  ...
  if (vlistaSelecionada.size() > 2 + (12 * (vpagina - 1))) {
    ...
    ui->labelFundoAnime02Progresso->setStyleSheet(
        "background: rgb(185,201,250);");
    ...
    ui->labelAnime02Titulo->setText(
        vlistaSelecionada[2 + (12 * (vpagina - 1))]->vnome);
    ...
    if (!carquivos
             ->fprocuraEpisodio(vlistaSelecionada[2 + (12 * (vpagina - 1))])
             .isEmpty() &&
        vtipoAtual == "anime")
      ui->labelAnime02Progresso->setStyleSheet(vfonteNormalAnimeEncontrado);
    else if (vlistaSelecionada[2 + (12 * (vpagina - 1))]->vstatus.compare(
                 "Not Aired Yet", Qt::CaseInsensitive) == 0)
      ui->labelAnime02Progresso->setStyleSheet(vfonteNormalAnimeNaoComecou);
    else
      ui->labelAnime02Progresso->setStyleSheet(vfonteNormal);
    ui->labelAnime02Progresso->setText(
        "Progresso: " +
        vlistaSelecionada[2 + (12 * (vpagina - 1))]->vnumEpisodiosAssistidos +
    ...
    ui->labelAnime02Nota->setText(
        "Nota: " +
        vlistaSelecionada[2 + (12 * (vpagina - 1))]->vnotaMediaPessoal + "/10");
    if (vlistaAtual == "busca" || vlistaAtual.contains("season")) {
      ui->labelFundoAnime02Lista->setStyleSheet(
          "background: rgb(185, 202, 250); font: 75 8pt \"Calibri\"; "
          "font-weight: bold; color: rgb(20, 20, 20);");
      bool ok;
      // Checa se a lista é um número válido
      ...
      if (ok)
        ui->labelFundoAnime02Lista->setText(
            vlistaSelecionada[2 + (12 * (vpagina - 1))]->vSeasonSemAno);
      else
        ui->labelFundoAnime02Lista->setText(
            vlistaSelecionada[2 + (12 * (vpagina - 1))]->vlista);
    } else {
    ...
    if (vimagemCarregada[vlistaSelecionada[2 + (12 * (vpagina - 1))]->vid]) {
      if (pix.load(cconfBase->vdiretorioImagensMedio +
                       vlistaSelecionada[2 + (12 * (vpagina - 1))]->vid +
                       ".jpg",
                   "jpg")) {
  ...
  if (vlistaSelecionada.size() > 3 + (12 * (vpagina - 1))) {
    ...
    ui->labelFundoAnime03Progresso->setStyleSheet(
        "background: rgb(185,201,250);");
    ...
    ui->labelAnime03Titulo->setText(
        vlistaSelecionada[3 + (12 * (vpagina - 1))]->vnome);
    ...
    if (!carquivos
             ->fprocuraEpisodio(vlistaSelecionada[3 + (12 * (vpagina - 1))])
             .isEmpty() &&
        vtipoAtual == "anime")
      ui->labelAnime03Progresso->setStyleSheet(vfonteNormalAnimeEncontrado);
    else if (vlistaSelecionada[3 + (12 * (vpagina - 1))]->vstatus.compare(
                 "Not Aired Yet", Qt::CaseInsensitive) == 0)
      ui->labelAnime03Progresso->setStyleSheet(vfonteNormalAnimeNaoComecou);
    else
      ui->labelAnime03Progresso->setStyleSheet(vfonteNormal);
    ui->labelAnime03Progresso->setText(
        "Progresso: " +
        vlistaSelecionada[3 + (12 * (vpagina - 1))]->vnumEpisodiosAssistidos +
    ...
    ui->labelAnime03Nota->setText(
        "Nota: " +
        vlistaSelecionada[3 + (12 * (vpagina - 1))]->vnotaMediaPessoal + "/10");
    if (vlistaAtual == "busca" || vlistaAtual.contains("season")) {
      ui->labelFundoAnime03Lista->setStyleSheet(
          "background: rgb(185, 202, 250); font: 75 8pt \"Calibri\"; "
          "font-weight: bold; color: rgb(20, 20, 20);");
      bool ok;
      // Checa se a lista é um número válido
      ...
      if (ok)
        ui->labelFundoAnime03Lista->setText(
            vlistaSelecionada[3 + (12 * (vpagina - 1))]->vSeasonSemAno);
      else
        ui->labelFundoAnime03Lista->setText(
            vlistaSelecionada[3 + (12 * (vpagina - 1))]->vlista);
    } else {
    ...
    if (vimagemCarregada[vlistaSelecionada[3 + (12 * (vpagina - 1))]->vid]) {
      if (pix.load(cconfBase->vdiretorioImagensMedio +
                       vlistaSelecionada[3 + (12 * (vpagina - 1))]->vid +
                       ".jpg",
                   "jpg")) {
  ...
  if (vlistaSelecionada.size() > 4 + (12 * (vpagina - 1))) {
    ...
    ui->labelFundoAnime04Progresso->setStyleSheet(
        "background: rgb(185,201,250);");
    ...
    ui->labelAnime04Titulo->setText(
        vlistaSelecionada[4 + (12 * (vpagina - 1))]->vnome);
    ...
    if (!carquivos
             ->fprocuraEpisodio(vlistaSelecionada[4 + (12 * (vpagina - 1))])
             .isEmpty() &&
        vtipoAtual == "anime")
      ui->labelAnime04Progresso->setStyleSheet(vfonteNormalAnimeEncontrado);
    else if (vlistaSelecionada[4 + (12 * (vpagina - 1))]->vstatus.compare(
                 "Not Aired Yet", Qt::CaseInsensitive) == 0)
      ui->labelAnime04Progresso->setStyleSheet(vfonteNormalAnimeNaoComecou);
    else
      ui->labelAnime04Progresso->setStyleSheet(vfonteNormal);
    ui->labelAnime04Progresso->setText(
        "Progresso: " +
        vlistaSelecionada[4 + (12 * (vpagina - 1))]->vnumEpisodiosAssistidos +
    ...
    ui->labelAnime04Nota->setText(
        "Nota: " +
        vlistaSelecionada[4 + (12 * (vpagina - 1))]->vnotaMediaPessoal + "/10");
    if (vlistaAtual == "busca" || vlistaAtual.contains("season")) {
      ui->labelFundoAnime04Lista->setStyleSheet(
          "background: rgb(185, 202, 250); font: 75 8pt \"Calibri\"; "
          "font-weight: bold; color: rgb(20, 20, 20);");
      bool ok;
      // Checa se a lista é um número válido
      ...
      if (ok)
        ui->labelFundoAnime04Lista->setText(
            vlistaSelecionada[4 + (12 * (vpagina - 1))]->vSeasonSemAno);
      else
        ui->labelFundoAnime04Lista->setText(
            vlistaSelecionada[4 + (12 * (vpagina - 1))]->vlista);
    } else {
    ...
    if (vimagemCarregada[vlistaSelecionada[4 + (12 * (vpagina - 1))]->vid]) {
      if (pix.load(cconfBase->vdiretorioImagensMedio +
                       vlistaSelecionada[4 + (12 * (vpagina - 1))]->vid +
                       ".jpg",
                   "jpg")) {
  ...
  if (vlistaSelecionada.size() > 5 + (12 * (vpagina - 1))) {
    ...
    ui->labelFundoAnime05Progresso->setStyleSheet(
        "background: rgb(185,201,250);");
    ...
    ui->labelAnime05Titulo->setText(
        vlistaSelecionada[5 + (12 * (vpagina - 1))]->vnome);
    ...
    ui->labelAnime05Progresso->setText(
        "Progresso: " +
        vlistaSelecionada[5 + (12 * (vpagina - 1))]->vnumEpisodiosAssistidos +
    ...
    if (!carquivos
             ->fprocuraEpisodio(vlistaSelecionada[5 + (12 * (vpagina - 1))])
             .isEmpty() &&
        vtipoAtual == "anime")
      ui->labelAnime05Progresso->setStyleSheet(vfonteNormalAnimeEncontrado);
    else if (vlistaSelecionada[5 + (12 * (vpagina - 1))]->vstatus.compare(
                 "Not Aired Yet", Qt::CaseInsensitive) == 0)
      ui->labelAnime05Progresso->setStyleSheet(vfonteNormalAnimeNaoComecou);
    else
      ui->labelAnime05Progresso->setStyleSheet(vfonteNormal);
    ui->labelAnime05Nota->setText(
        "Nota: " +
        vlistaSelecionada[5 + (12 * (vpagina - 1))]->vnotaMediaPessoal + "/10");
    if (vlistaAtual == "busca" || vlistaAtual.contains("season")) {
      ui->labelFundoAnime05Lista->setStyleSheet(
          "background: rgb(185, 202, 250); font: 75 8pt \"Calibri\"; "
          "font-weight: bold; color: rgb(20, 20, 20);");
      bool ok;
      // Checa se a lista é um número válido
      ...
      if (ok)
        ui->labelFundoAnime05Lista->setText(
            vlistaSelecionada[5 + (12 * (vpagina - 1))]->vSeasonSemAno);
      else
        ui->labelFundoAnime05Lista->setText(
            vlistaSelecionada[5 + (12 * (vpagina - 1))]->vlista);
    } else {
    ...
    if (vimagemCarregada[vlistaSelecionada[5 + (12 * (vpagina - 1))]->vid]) {
      if (pix.load(cconfBase->vdiretorioImagensMedio +
                       vlistaSelecionada[5 + (12 * (vpagina - 1))]->vid +
                       ".jpg",
                   "jpg")) {
  ...
  if (vlistaSelecionada.size() > 6 + (12 * (vpagina - 1))) {
    ...
    ui->labelFundoAnime06Progresso->setStyleSheet(
        "background: rgb(185,201,250);");
    ...
    ui->labelAnime06Titulo->setText(
        vlistaSelecionada[6 + (12 * (vpagina - 1))]->vnome);
    ...
    ui->labelAnime06Progresso->setText(
        "Progresso: " +
        vlistaSelecionada[6 + (12 * (vpagina - 1))]->vnumEpisodiosAssistidos +
    ...
    if (!carquivos
             ->fprocuraEpisodio(vlistaSelecionada[6 + (12 * (vpagina - 1))])
             .isEmpty() &&
        vtipoAtual == "anime")
      ui->labelAnime06Progresso->setStyleSheet(vfonteNormalAnimeEncontrado);
    else if (vlistaSelecionada[6 + (12 * (vpagina - 1))]->vstatus.compare(
                 "Not Aired Yet", Qt::CaseInsensitive) == 0)
      ui->labelAnime06Progresso->setStyleSheet(vfonteNormalAnimeNaoComecou);
    else
      ui->labelAnime06Progresso->setStyleSheet(vfonteNormal);
    ui->labelAnime06Nota->setText(
        "Nota: " +
        vlistaSelecionada[6 + (12 * (vpagina - 1))]->vnotaMediaPessoal + "/10");
    if (vlistaAtual == "busca" || vlistaAtual.contains("season")) {
      ui->labelFundoAnime06Lista->setStyleSheet(
          "background: rgb(185, 202, 250); font: 75 8pt \"Calibri\"; "
          "font-weight: bold; color: rgb(20, 20, 20);");
      bool ok;
      // Checa se a lista é um número válido
      ...
      if (ok)
        ui->labelFundoAnime06Lista->setText(
            vlistaSelecionada[6 + (12 * (vpagina - 1))]->vSeasonSemAno);
      else
        ui->labelFundoAnime06Lista->setText(
            vlistaSelecionada[6 + (12 * (vpagina - 1))]->vlista);
    } else {
    ...
    if (vimagemCarregada[vlistaSelecionada[6 + (12 * (vpagina - 1))]->vid]) {
      if (pix.load(cconfBase->vdiretorioImagensMedio +
                       vlistaSelecionada[6 + (12 * (vpagina - 1))]->vid +
                       ".jpg",
                   "jpg")) {
  ...
  if (vlistaSelecionada.size() > 7 + (12 * (vpagina - 1))) {
    ...
    ui->labelFundoAnime07Progresso->setStyleSheet(
        "background: rgb(185,201,250);");
    ...
    ui->labelAnime07Titulo->setText(
        vlistaSelecionada[7 + (12 * (vpagina - 1))]->vnome);
    ...
    if (!carquivos
             ->fprocuraEpisodio(vlistaSelecionada[7 + (12 * (vpagina - 1))])
             .isEmpty() &&
        vtipoAtual == "anime")
      ui->labelAnime07Progresso->setStyleSheet(vfonteNormalAnimeEncontrado);
    else if (vlistaSelecionada[7 + (12 * (vpagina - 1))]->vstatus.compare(
                 "Not Aired Yet", Qt::CaseInsensitive) == 0)
      ui->labelAnime07Progresso->setStyleSheet(vfonteNormalAnimeNaoComecou);
    else
      ui->labelAnime07Progresso->setStyleSheet(vfonteNormal);
    ui->labelAnime07Progresso->setText(
        "Progresso: " +
        vlistaSelecionada[7 + (12 * (vpagina - 1))]->vnumEpisodiosAssistidos +
    ...
    ui->labelAnime07Nota->setText(
        "Nota: " +
        vlistaSelecionada[7 + (12 * (vpagina - 1))]->vnotaMediaPessoal + "/10");
    if (vlistaAtual == "busca" || vlistaAtual.contains("season")) {
      ui->labelFundoAnime07Lista->setStyleSheet(
          "background: rgb(185, 202, 250); font: 75 8pt \"Calibri\"; "
          "font-weight: bold; color: rgb(20, 20, 20);");
      bool ok;
      // Checa se a lista é um número válido
      ...
      if (ok)
        ui->labelFundoAnime07Lista->setText(
            vlistaSelecionada[7 + (12 * (vpagina - 1))]->vSeasonSemAno);
      else
        ui->labelFundoAnime07Lista->setText(
            vlistaSelecionada[7 + (12 * (vpagina - 1))]->vlista);
    } else {
    ...
    if (vimagemCarregada[vlistaSelecionada[7 + (12 * (vpagina - 1))]->vid]) {
      if (pix.load(cconfBase->vdiretorioImagensMedio +
                       vlistaSelecionada[7 + (12 * (vpagina - 1))]->vid +
                       ".jpg",
                   "jpg")) {
  ...
  if (vlistaSelecionada.size() > 8 + (12 * (vpagina - 1))) {
    ...
    ui->labelFundoAnime08Progresso->setStyleSheet(
        "background: rgb(185,201,250);");
    ...
    ui->labelAnime08Titulo->setText(
        vlistaSelecionada[8 + (12 * (vpagina - 1))]->vnome);
    ...
    if (!carquivos
             ->fprocuraEpisodio(vlistaSelecionada[8 + (12 * (vpagina - 1))])
             .isEmpty() &&
        vtipoAtual == "anime")
      ui->labelAnime08Progresso->setStyleSheet(vfonteNormalAnimeEncontrado);
    else if (vlistaSelecionada[8 + (12 * (vpagina - 1))]->vstatus.compare(
                 "Not Aired Yet", Qt::CaseInsensitive) == 0)
      ui->labelAnime08Progresso->setStyleSheet(vfonteNormalAnimeNaoComecou);
    else
      ui->labelAnime08Progresso->setStyleSheet(vfonteNormal);
    ui->labelAnime08Progresso->setText(
        "Progresso: " +
        vlistaSelecionada[8 + (12 * (vpagina - 1))]->vnumEpisodiosAssistidos +
    ...
    ui->labelAnime08Nota->setText(
        "Nota: " +
        vlistaSelecionada[8 + (12 * (vpagina - 1))]->vnotaMediaPessoal + "/10");
    if (vlistaAtual == "busca" || vlistaAtual.contains("season")) {
      ui->labelFundoAnime08Lista->setStyleSheet(
          "background: rgb(185, 202, 250); font: 75 8pt \"Calibri\"; "
          "font-weight: bold; color: rgb(20, 20, 20);");
      bool ok;
      // Checa se a lista é um número válido
      ...
      if (ok)
        ui->labelFundoAnime08Lista->setText(
            vlistaSelecionada[8 + (12 * (vpagina - 1))]->vSeasonSemAno);
      else
        ui->labelFundoAnime08Lista->setText(
            vlistaSelecionada[8 + (12 * (vpagina - 1))]->vlista);
    } else {
    ...
    if (vimagemCarregada[vlistaSelecionada[8 + (12 * (vpagina - 1))]->vid]) {
      if (pix.load(cconfBase->vdiretorioImagensMedio +
                       vlistaSelecionada[8 + (12 * (vpagina - 1))]->vid +
                       ".jpg",
                   "jpg")) {
  ...
  if (vlistaSelecionada.size() > 9 + (12 * (vpagina - 1))) {
    ...
    ui->labelFundoAnime09Progresso->setStyleSheet(
        "background: rgb(185,201,250);");
    ...
    ui->labelAnime09Titulo->setText(
        vlistaSelecionada[9 + (12 * (vpagina - 1))]->vnome);
    ...
    if (!carquivos
             ->fprocuraEpisodio(vlistaSelecionada[9 + (12 * (vpagina - 1))])
             .isEmpty() &&
        vtipoAtual == "anime")
      ui->labelAnime09Progresso->setStyleSheet(vfonteNormalAnimeEncontrado);
    else if (vlistaSelecionada[9 + (12 * (vpagina - 1))]->vstatus.compare(
                 "Not Aired Yet", Qt::CaseInsensitive) == 0)
      ui->labelAnime09Progresso->setStyleSheet(vfonteNormalAnimeNaoComecou);
    else
      ui->labelAnime09Progresso->setStyleSheet(vfonteNormal);
    ui->labelAnime09Progresso->setText(
        "Progresso: " +
        vlistaSelecionada[9 + (12 * (vpagina - 1))]->vnumEpisodiosAssistidos +
    ...
    ui->labelAnime09Nota->setText(
        "Nota: " +
        vlistaSelecionada[9 + (12 * (vpagina - 1))]->vnotaMediaPessoal + "/10");
    if (vlistaAtual == "busca" || vlistaAtual.contains("season")) {
      ui->labelFundoAnime09Lista->setStyleSheet(
          "background: rgb(185, 202, 250); font: 75 8pt \"Calibri\"; "
          "font-weight: bold; color: rgb(20, 20, 20);");
      bool ok;
      // Checa se a lista é um número válido
      ...
      if (ok)
        ui->labelFundoAnime09Lista->setText(
            vlistaSelecionada[9 + (12 * (vpagina - 1))]->vSeasonSemAno);
      else
        ui->labelFundoAnime09Lista->setText(
            vlistaSelecionada[9 + (12 * (vpagina - 1))]->vlista);
    } else {
    ...
    if (vimagemCarregada[vlistaSelecionada[9 + (12 * (vpagina - 1))]->vid]) {
      if (pix.load(cconfBase->vdiretorioImagensMedio +
                       vlistaSelecionada[9 + (12 * (vpagina - 1))]->vid +
                       ".jpg",
                   "jpg")) {
  ...
  if (vlistaSelecionada.size() > 10 + (12 * (vpagina - 1))) {
    ...
    ui->labelFundoAnime10Progresso->setStyleSheet(
        "background: rgb(185,201,250);");
    ...
    ui->labelAnime10Titulo->setText(
        vlistaSelecionada[10 + (12 * (vpagina - 1))]->vnome);
    ...
    if (!carquivos
             ->fprocuraEpisodio(vlistaSelecionada[10 + (12 * (vpagina - 1))])
             .isEmpty() &&
        vtipoAtual == "anime")
      ui->labelAnime10Progresso->setStyleSheet(vfonteNormalAnimeEncontrado);
    else if (vlistaSelecionada[10 + (12 * (vpagina - 1))]->vstatus.compare(
                 "Not Aired Yet", Qt::CaseInsensitive) == 0)
      ui->labelAnime10Progresso->setStyleSheet(vfonteNormalAnimeNaoComecou);
    else
      ui->labelAnime10Progresso->setStyleSheet(vfonteNormal);
    ui->labelAnime10Progresso->setText(
        "Progresso: " +
        vlistaSelecionada[10 + (12 * (vpagina - 1))]->vnumEpisodiosAssistidos +
        "/" +
    ...
    ui->labelAnime10Nota->setText(
        "Nota: " +
        vlistaSelecionada[10 + (12 * (vpagina - 1))]->vnotaMediaPessoal +
        "/10");
    if (vlistaAtual == "busca" || vlistaAtual.contains("season")) {
      ui->labelFundoAnime10Lista->setStyleSheet(
          "background: rgb(185, 202, 250); font: 75 8pt \"Calibri\"; "
          "font-weight: bold; color: rgb(20, 20, 20);");
      bool ok;
      // Checa se a lista é um número válido
      ...
      if (ok)
        ui->labelFundoAnime10Lista->setText(
            vlistaSelecionada[10 + (12 * (vpagina - 1))]->vSeasonSemAno);
      else
        ui->labelFundoAnime10Lista->setText(
            vlistaSelecionada[10 + (12 * (vpagina - 1))]->vlista);
    } else {
    ...
    if (vimagemCarregada[vlistaSelecionada[10 + (12 * (vpagina - 1))]->vid]) {
      if (pix.load(cconfBase->vdiretorioImagensMedio +
                       vlistaSelecionada[10 + (12 * (vpagina - 1))]->vid +
                       ".jpg",
                   "jpg")) {
        ui->imagemAnime10->setPixmap(pix);
      } else if (pix.load(
                     cconfBase->vdiretorioImagensMedio +
                         vlistaSelecionada[10 + (12 * (vpagina - 1))]->vid +
                         ".png",
                     "png")) {
        ui->imagemAnime10->setPixmap(pix);
  ...
  if (vlistaSelecionada.size() > 11 + (12 * (vpagina - 1))) {
    ...
    ui->labelFundoAnime11Progresso->setStyleSheet(
        "background: rgb(185,201,250);");
    ...
    ui->labelAnime11Titulo->setText(
        vlistaSelecionada[11 + (12 * (vpagina - 1))]->vnome);
    ...
    if (!carquivos
             ->fprocuraEpisodio(vlistaSelecionada[11 + (12 * (vpagina - 1))])
             .isEmpty() &&
        vtipoAtual == "anime")
      ui->labelAnime11Progresso->setStyleSheet(vfonteNormalAnimeEncontrado);
    else if (vlistaSelecionada[11 + (12 * (vpagina - 1))]->vstatus.compare(
                 "Not Aired Yet", Qt::CaseInsensitive) == 0)
      ui->labelAnime11Progresso->setStyleSheet(vfonteNormalAnimeNaoComecou);
    else
      ui->labelAnime11Progresso->setStyleSheet(vfonteNormal);
    ui->labelAnime11Progresso->setText(
        "Progresso: " +
        vlistaSelecionada[11 + (12 * (vpagina - 1))]->vnumEpisodiosAssistidos +
        "/" +
    ...
    ui->labelAnime11Nota->setText(
        "Nota: " +
        vlistaSelecionada[11 + (12 * (vpagina - 1))]->vnotaMediaPessoal +
        "/10");
    if (vlistaAtual == "busca" || vlistaAtual.contains("season")) {
      ui->labelFundoAnime11Lista->setStyleSheet(
          "background: rgb(185, 202, 250); font: 75 8pt \"Calibri\"; "
          "font-weight: bold; color: rgb(20, 20, 20);");
      bool ok;
      // Checa se a lista é um número válido
      ...
      if (ok)
        ui->labelFundoAnime11Lista->setText(
            vlistaSelecionada[11 + (12 * (vpagina - 1))]->vSeasonSemAno);
      else
        ui->labelFundoAnime11Lista->setText(
            vlistaSelecionada[11 + (12 * (vpagina - 1))]->vlista);
    } else {
    ...
    if (vimagemCarregada[vlistaSelecionada[11 + (12 * (vpagina - 1))]->vid]) {
      if (pix.load(cconfBase->vdiretorioImagensMedio +
                       vlistaSelecionada[11 + (12 * (vpagina - 1))]->vid +
                       ".jpg",
                   "jpg")) {
...
bool MainWindow::fcarregaImagensBackground(const QString &lista) {
  ...
  if (!vcarregaImagens.isRunning() && vdownloadImagensAcabou)
    vcarregaImagens = QtConcurrent::run(
        this, &MainWindow::fcarregaImagensSelecionadasBackground);
  return true;
}

// Dá load em todas as imagens disponíveis no computador para não ter que
// carregar elas depois
      ...
      if (!vusarImagensBaixaQualidade) {
        if (!vimagemCarregada[id] && vimagemBaixada[id]) {
          ...
          if (vprogressoLoadingBar != vtotalAnimesLoadingBar)
            vprogressoLoadingBar++;
          ui->labelLoading->setText(
              "Loaded " + QString::number(vprogressoLoadingBar) + " of " +
              QString::number(vtotalAnimesLoadingBar) + " images");
        }
      } else {
...
void MainWindow::on_botaoProximaPagina_clicked() {
  ...
  if (vlistaSelecionada.size() > 12 + (12 * (vpagina - 1))) {
    ...
    if (vlistaAtual == "watching")
      ...
      llistaAtual = "Busca";
    ui->NumPagina->setText(
        llistaAtual + " - " + QString::number(vlistaSelecionada.size()) +
        " animes in the list - Page " + QString::number(vpagina) + "/" +
        QString::number(((vlistaSelecionada.size() - 1) / 12) + 1));
    fcarregaImagensLista();
  }
...
void MainWindow::on_Watching_clicked() {
  ...
  if (vJanelaManga) {
    ...
    ui->NumPagina->setText(
        "Reading - " + QString::number(vlistaSelecionada.size()) +
        " mangas in the list - Page " + QString::number(vpagina) + "/" +
        QString::number(((vlistaSelecionada.size() - 1) / 12) + 1));
    ...
    ui->NumPagina->setText(
        "Reading - " + QString::number(vlistaSelecionada.size()) +
        " novels in the list - Page " + QString::number(vpagina) + "/" +
        QString::number(((vlistaSelecionada.size() - 1) / 12) + 1));
...
void MainWindow::on_Completed_clicked() {
  ...
  if (vJanelaManga) {
    ...
    ui->NumPagina->setText(
        "Completed - " + QString::number(vlistaSelecionada.size()) +
        " mangas in the list - Page " + QString::number(vpagina) + "/" +
        QString::number(((vlistaSelecionada.size() - 1) / 12) + 1));
  } else if (vJanelaNovel) {
...
void MainWindow::on_OnHold_clicked() {
  ...
  if (vJanelaManga) {
    ...
    ui->NumPagina->setText(
        "On Hold - " + QString::number(vlistaSelecionada.size()) +
        " mangas in the list - Page " + QString::number(vpagina) + "/" +
        QString::number(((vlistaSelecionada.size() - 1) / 12) + 1));
  } else if (vJanelaNovel) {
...
void MainWindow::on_Dropped_clicked() {
  ...
  if (vJanelaManga) {
    ...
    ui->NumPagina->setText(
        "Dropped - " + QString::number(vlistaSelecionada.size()) +
        " mangas in the list - Page " + QString::number(vpagina) + "/" +
        QString::number(((vlistaSelecionada.size() - 1) / 12) + 1));
  } else if (vJanelaNovel) {
...
void MainWindow::on_PlanToWatch_clicked() {
  ...
  if (vJanelaManga) {
    vlistaSelecionada =
        cleitorListaAnimes->sortLista(vordem, "mangaplantoread");
    vlistaAtual = "mangaplantoread";
    ui->NumPagina->setText(
        "Plan to Read - " + QString::number(vlistaSelecionada.size()) +
        " mangas in the list - Page " + QString::number(vpagina) + "/" +
        QString::number(((vlistaSelecionada.size() - 1) / 12) + 1));
  } else if (vJanelaNovel) {
    vlistaSelecionada =
        cleitorListaAnimes->sortLista(vordem, "novelplantoread");
    ...
    ui->NumPagina->setText(
        "Plan to Watch - " + QString::number(vlistaSelecionada.size()) +
        " animes in the list - Page " + QString::number(vpagina) + "/" +
        QString::number(((vlistaSelecionada.size() - 1) / 12) + 1));
  }
  finfoAnimeSelecionado();
...
void MainWindow::on_botaoProximoEpisodio_clicked() {
  ...
  if (!carquivos->fprocuraEpisodio(vlistaSelecionada[vanimeSelecionado])
           .isEmpty()) {
    if (carquivos->fabreEpisodio(
            carquivos->fprocuraEpisodio(vlistaSelecionada[vanimeSelecionado])
                .toUtf8()))
...
void MainWindow::on_botaoAbrePasta_clicked() {
  ...
  // Pega o caminho da pasta onde o episódio está e converte para o caminho
  // nativo do OS (só pra evitar erros com o nome)
  QByteArray lcaminhoPasta =
      cconfUsuario
          ->fretornaDiretorioEspecifico(
              vlistaSelecionada[vanimeSelecionado]->vid.toInt())
          .toUtf8();
...
void MainWindow::on_botaoBusca_clicked() {
  if (ui->janelaRotativa->currentIndex() == 0) {
    if (!ui->barraBusca->toPlainText().isEmpty()) {
      ...
      vlistaSelecionada = cleitorListaAnimes->fbuscaLista(
          ui->barraBusca->toPlainText().simplified(), vtipoAtual);
      if (!vlistaSelecionada.isEmpty()) {
        vlistaAtual = "busca";
        vanimeSelecionado = 0;
        vpagina = 1;
        ui->NumPagina->setText(
            "Busca  - " + QString::number(vlistaSelecionada.size()) +
            " animes in the list - Page " + QString::number(vpagina) + "/" +
            QString::number(((vlistaSelecionada.size() - 1) / 12) + 1));
        finfoAnimeSelecionado();
      } else {
        qDebug() << ui->barraBusca->toPlainText() << " not found!";
        ui->labelMensagem->setText(ui->barraBusca->toPlainText() +
                                   " not found!");
        vlistaSelecionada = cleitorListaAnimes->sortLista(vordem, vlistaAtual);
      }
      ...
      vlistaSelecionada =
          cleitorListaAnimes->sortLista(vordem, vlistaAtual + vtipoAtual);
      if (!vlistaSelecionada.isEmpty()) {
        ui->NumPagina->setText(
            vtipoAtual + " " + tempSeason.remove("season") + "  - " +
            QString::number(vlistaSelecionada.size()) +
            " animes in the list - Page " + QString::number(vpagina) + "/" +
            QString::number(((vlistaSelecionada.size() - 1) / 12) + 1));
        ...
        ui->labelMensagem->setText(
            vtipoAtual + " " + vlistaAtual.remove("season") + " not found!");
        vlistaSelecionada = cleitorListaAnimes->sortLista(vordem, vlistaAtual);
      }
    }
...
void MainWindow::on_botaoRefresh_clicked() {
  if (!vrefreshAcontecendo) {
    ui->labelMensagem->setText("Trying to update list. The program may freeze "
                               "for a few seconds while reading the new list");
...
void MainWindow::frefreshListas(bool rcheckDownload) {
  // Checamos se a conexão com o anilist foi um sucesso. No caso de ter falhado,
  // não rodamos a função de refresh
  if (!rcheckDownload) {
  ...
  // Quando rolar a busca, a lista vai resetar. Esse index será usado pra voltar
  // na info do anime que a pessoa estava vendo quando rolou o refresh
  int lanimeAtual = 0;
  int lpaginaAtual = 0;
  bool lbusca = false;
  rcheckDownload = false;
  // Vamos bloquear todos os botões para evitar que o usuário tente ler um anime
  // que saiu da lista
  ui->labelMensagem->setText(
      "Loading new animes. It will be working again in a few seconds");
  fbloqueiaSinaisBotoes();

  qDebug() << "Refreshing list...";

  // Se não houver nenhum problema no download ou no caso do usuário deletar o
  // arquivo das listas, elas vão estar sempre prontas
  if (vlistaLidaSucesso) {
  ...
  // Caso o usuário esteja no meio de uma busca quando atualiza a lista,
  // precisamos salvar o index do anime que ele estava vendo.
  ...
  ui->labelMensagem->setText(
      "The lists were updated! Searching animes in the computer.");
  fliberaSinaisBotoes();
  vrefreshAcontecendo = false;
...
void MainWindow::fbloqueiaSinaisBotoes() {
  QList<QPushButton *> listaBotoes =
      ui->listaAnimes->findChildren<QPushButton *>();
  foreach (QPushButton *psh, listaBotoes) { psh->blockSignals(true); }
}

void MainWindow::fliberaSinaisBotoes() {
  QList<QPushButton *> listaBotoes =
      ui->listaAnimes->findChildren<QPushButton *>();
  foreach (QPushButton *psh, listaBotoes) { psh->blockSignals(false); }
}
...
void MainWindow::fatualizaRefreshTimer() {
  ...
  if (!vrefreshAcontecendo) {
    ...
    if (vtimerSegundos == 0)
      ...
      ui->labelRefreshTimer->setText(QString::number(lminutos) + ":0" +
                                     QString::number(vtimerSegundos));
    else
      ui->labelRefreshTimer->setText(QString::number(lminutos) + ":" +
                                     QString::number(vtimerSegundos));
    if (vtimerSegundos == 0)
      vtimerSegundos = 59;
    else
...
void MainWindow::on_botaoNotaMais_clicked() {
  if (vlistaSelecionada[vanimeSelecionado]->vnotaMediaPessoal.toInt() < 10) {
    vlistaSelecionada[vanimeSelecionado]->vnotaMediaPessoal = QString::number(
        vlistaSelecionada[vanimeSelecionado]->vnotaMediaPessoal.toInt() + 1);
    ui->labelInfoNota->setText(
        vlistaSelecionada[vanimeSelecionado]->vnotaMediaPessoal);
    cleitorListaAnimes->fmudaNota(
        vlistaSelecionada[vanimeSelecionado]->vid,
        vlistaSelecionada[vanimeSelecionado]->vnotaMediaPessoal);
    finfoAnimeSelecionado();
    ...
    vlistaAcoes.insert(
        lstringListAcao,
        QString::number(
            vlistaSelecionada[vanimeSelecionado]->vnotaMediaPessoal.toInt()));
  }
}
...
void MainWindow::on_botaoProgressoMais_clicked() {
  ...
  // Caso o número máximo de episódios não seja conhecido, não deve existir um
  // limite;
  ...
  if (vlistaSelecionada[vanimeSelecionado]->vnumEpisodiosAssistidos.toInt() <
    ...
    vlistaSelecionada[vanimeSelecionado]->vnumEpisodiosAssistidos =
        QString::number(vlistaSelecionada[vanimeSelecionado]
                            ->vnumEpisodiosAssistidos.toInt() +
                        1);
    cleitorListaAnimes->fmudaProgresso(
        vlistaSelecionada[vanimeSelecionado]->vid,
        vlistaSelecionada[vanimeSelecionado]->vnumEpisodiosAssistidos);
    finfoAnimeSelecionado();
    ...
    } else if ((vlistaSelecionada[vanimeSelecionado]->vlista.compare(
                    "Completed", Qt::CaseInsensitive) != 0)) {

      vlistaSelecionada[vanimeSelecionado]->vlista = "Watchig";
      canilist->fmudaLista(vlistaSelecionada[vanimeSelecionado]->vid.toInt(),
                           "CURRENT");
      cleitorListaAnimes->fmudaLista(
          vlistaSelecionada[vanimeSelecionado]->vid,
...
void MainWindow::fAumentaProgressoID(const QString &ridAnime,
                                     const QString &repisodioAnime) {
  fbloqueiaSinaisBotoes();
  ...
  QString llistaAnimeAssistindo =
      cleitorListaAnimes->fbuscaAnimePorIDERetornaLista(ridAnime);
  // A função vai rodar tudo em uma lista temporária e dar refresh na lista
  // atual, pra mudar o progresso
  QVector<anime *> listaTemp;
  if (llistaAnimeAssistindo.isEmpty())
    return;
  else {
    // Olha todas as listas, exceto completed. Não tem sentido aumentar o
    // progresso de algo já completo.
    if (llistaAnimeAssistindo.compare("Watching", Qt::CaseInsensitive) == 0) {
      listaTemp = cleitorListaAnimes->retornaListaWatching();
    } else if (llistaAnimeAssistindo.compare("On Hold", Qt::CaseInsensitive) ==
               0) {
      listaTemp = cleitorListaAnimes->retornaListaOnHold();
    } else if (llistaAnimeAssistindo.compare("Dropped", Qt::CaseInsensitive) ==
               0) {
      listaTemp = cleitorListaAnimes->retornaListaDropped();
    } else if (llistaAnimeAssistindo.compare("Plan to Watch",
      ...
      if (ok) {
        listaTemp =
            cleitorListaAnimes->fleListaAno(llistaAnimeAssistindo.toInt());
      }
    }
  }
  if (lposicao != -1 && listaTemp[lposicao]->vnumEpisodiosAssistidos.toInt() <
                            repisodioAnime.toInt()) {
    for (int i = 0;
         i < repisodioAnime.toInt() -
                 listaTemp[lposicao]->vnumEpisodiosAssistidos.toInt();
         i++) {
      int lepisodiosTotais = 0;
      // Caso o número máximo de episódios não seja conhecido, não deve existir
      // um limite;
      ...
      if (listaTemp[lposicao]->vnumEpisodiosAssistidos.toInt() <
          lepisodiosTotais) {
        listaTemp[lposicao]->vnumEpisodiosAssistidos = QString::number(
            listaTemp[lposicao]->vnumEpisodiosAssistidos.toInt() + 1);
        cleitorListaAnimes->fmudaProgresso(
            listaTemp[i]->vid, listaTemp[i]->vnumEpisodiosAssistidos);
        finfoAnimeSelecionado();
        ...
        // Caso esteja vendo um anime de, por exemplo, plan to watch, manda pra
        // lista watching Se estiver vendo um anime que não está na lista, ele é
        // adicionado em Watching também.
        else if ((llistaAnimeAssistindo.compare("Completed",
                                                Qt::CaseInsensitive) != 0)) {

          listaTemp[lposicao]->vlista = "Watchig";
          canilist->fmudaLista(listaTemp[lposicao]->vid.toInt(), "CURRENT");
          cleitorListaAnimes->fmudaLista(listaTemp[lposicao]->vid,
                                         listaTemp[lposicao]->vlista,
                                         vtipoAtual);
        }
      }
    }
  ...
  if (llistaAnimeAssistindo.compare("On Hold", Qt::CaseInsensitive) == 0) {
    ...
    vlistaSelecionada = cleitorListaAnimes->sortLista(vordem, "dropped");
                                           ...
                                           Qt::CaseInsensitive) == 0) {
    vlistaSelecionada = cleitorListaAnimes->sortLista(vordem, "plantowatch");
  } else if (llistaAnimeAssistindo.compare("Completed", Qt::CaseInsensitive) ==
             0) {
    vlistaSelecionada = cleitorListaAnimes->sortLista(vordem, "completed");
  } else {
    vlistaSelecionada = cleitorListaAnimes->sortLista(vordem, "watching");
...
void MainWindow::fgetConfigurations() {
  ...
  if (jconfig.fretornaDownloadAutomatico().contains("yes")) {
    ...
    connect(timerTorrent, &QTimer::timeout, &jtorrent,
            QOverload<>::of(&janelatorrent::fautoDownload));
  }
  fatualizaFontes();
}
...
void MainWindow::fsetDownloadImagensAnimesPorAno() {
  ...
  ui->labelMensagem->setText("Downloading and updating images. It can take a "
                             "few minutes to download every image");
...
void MainWindow::fsalvaNomesAlternativos() {
  ...
  if (larquivo.open(QIODevice::WriteOnly)) {
    ...
    foreach (QString key, vcustomNomesAlternativos.keys()) {
      lstreamTexto << key << ";"
                   << vcustomNomesAlternativos[key].join(";").trimmed() << endl;
    }
    larquivo.close();
  }
}
...
void MainWindow::fresetaRequestCounter() {
  vrateLimitRequestsAnilist = 0;
  timerMaxAnilistRequest.singleShot(61000, this,
                                    &MainWindow::fresetaRequestCounter);
}
  ...
  foreach (QStringList key, vlistaAcoes.keys()) {
    ...
    if (vrateLimitRequestsAnilist == 90)
      ...
      if (canilist->fmudaNota(key.at(1).toInt(), vlistaAcoes[key].toInt())) {
        qDebug() << QString("Change: Score - " + nome + " - " +
                            vlistaAcoes[key] + " - " +
                            QDateTime::currentDateTime().toString());
        vhistoricoAnilist.append(
            QString("Change: Score - " + nome + " - " + vlistaAcoes[key] +
                    " - " + QDateTime::currentDateTime().toString()));
        vlistaAcoes.remove(key);
      } else
        qDebug()
            << "Couldn't reach anilist server. Trying again in 10 seconds.";
    } else if (key.at(0) == "progresso") {
      if (canilist->fmudaProgresso(key.at(1).toInt(),
                                   vlistaAcoes[key].toInt())) {
        qDebug() << QString("Change: Progress - " +
                            vlistaSelecionada[vanimeSelecionado]->vnome +
                            " - " + vlistaAcoes[key] + " - " +
                            QDateTime::currentDateTime().toString());
        vhistoricoAnilist.append(
            QString("Change: Progress - " +
                    vlistaSelecionada[vanimeSelecionado]->vnome + " - " +
                    vlistaAcoes[key] + " - " +
                    QDateTime::currentDateTime().toString()));
        vlistaAcoes.remove(key);
      ...
      if (canilist->fmudaLista(key.at(1).toInt(), vlistaAcoes[key])) {
        qDebug() << QString("Change: List - " +
                            vlistaSelecionada[vanimeSelecionado]->vnome +
                            " - " + vlistaAcoes[key] + " - " +
                            QDateTime::currentDateTime().toString());
        vhistoricoAnilist.append(QString(
            "Change: List - " + vlistaSelecionada[vanimeSelecionado]->vnome +
            " - " + vlistaAcoes[key] + " - " +
            QDateTime::currentDateTime().toString()));
        vlistaAcoes.remove(key);
      } else
      ...
      if (canilist->fexcluiAnime(key.at(1).toInt())) {
        qDebug() << QString("Change: Remove - " +
                            vlistaSelecionada[vanimeSelecionado]->vnome +
                            " - " + vlistaAcoes[key] + " - " +
                            QDateTime::currentDateTime().toString());
        vhistoricoAnilist.append(QString(
            "Change: Remove - " + vlistaSelecionada[vanimeSelecionado]->vnome +
            " - " + vlistaAcoes[key] + " - " +
            QDateTime::currentDateTime().toString()));
        vlistaAcoes.remove(key);
      } else
...
void MainWindow::on_botaoMudarPraLista_clicked() {
  ...
  if (ui->boxMudarPraLista->currentText().compare(vlistaAtual,
                                                  Qt::CaseInsensitive) == 0)
    ...
    cleitorListaAnimes->fmudaLista(vlistaSelecionada[vanimeSelecionado]->vid,
                                   vlistaSelecionada[vanimeSelecionado]->vlista,
                                   vtipoAtual);
  } else if (ui->boxMudarPraLista->currentText().compare("Reading") == 0 &&
             vtipoAtual.compare("manga") == 0) {
    ...
    cleitorListaAnimes->fmudaLista(vlistaSelecionada[vanimeSelecionado]->vid,
                                   vlistaSelecionada[vanimeSelecionado]->vlista,
                                   vtipoAtual);
  } else if (ui->boxMudarPraLista->currentText().compare("Plan to Watch") ==
             0) {
    ...
    cleitorListaAnimes->fmudaLista(vlistaSelecionada[vanimeSelecionado]->vid,
                                   vlistaSelecionada[vanimeSelecionado]->vlista,
                                   vtipoAtual);
  } else if (ui->boxMudarPraLista->currentText().compare("Plan to Read") == 0 &&
             vtipoAtual.compare("manga") == 0) {
...
void MainWindow::on_botaoAnilist_clicked() {
  QDesktopServices::openUrl(
      QUrl(vlistaSelecionada[vanimeSelecionado]->vsiteAnilist));
}

void MainWindow::on_botaoCrunchyroll_clicked() {
  if (!QDesktopServices::openUrl(
          QUrl(vlistaSelecionada[vanimeSelecionado]->vstreamCrunchyroll)))
...
void MainWindow::on_botaoRemoverdaLista_clicked() {
  qDebug() << "Removing " << vlistaSelecionada[vanimeSelecionado]->vnome
           << "from list";
  vordem = "";
...
void MainWindow::on_botaoOrdemAlfabetica_clicked() {
  ...
  if (!vlistaSelecionada.isEmpty()) {
    ...
    vpagina = 1;
    ui->NumPagina->setText(
        vlistaAtual + " - " + QString::number(vlistaSelecionada.size()) +
        " animes in the list - Page " + QString::number(vpagina) + "/" +
        QString::number(((vlistaSelecionada.size() - 1) / 12) + 1));
    finfoAnimeSelecionado();
  }
...
void MainWindow::fmudaResolucao() {
  ...
  // fazer isso para não deixar a janela ficar atrás do menu iniciar. O windows
  // aceita a janela ficar atrás, mas imagino que terão OS que não irão aceitar.
  int objheight =
  ...
  // Essa função funciona muito bem, mas algumas coisas ficam com 1 ou 2 pixels
  // fora do lugar
  int widheight =
      qCeil(static_cast<int>(height * (static_cast<double>(821) / 1080)));
  ...
  // Pego a posição e o tamanho do label que vou usar como referência, já com a
  // nova resolução
  double posxLabelAnime =
      static_cast<double>(ui->imagemAnime00->pos().rx()) / 1920;
  double posyLabelAnime =
      static_cast<double>(ui->imagemAnime00->pos().ry()) / 821;
  ...
  ui->imagemAnime00->resize(
      static_cast<int>(comprimentoJanela * larguraLabelAnime),
  ...
  ui->labelFundoAnime00Titulo->resize(
      static_cast<int>(comprimentoJanela *
                       ui->labelFundoAnime00Titulo->width()),
      static_cast<int>(alturaJanela * ui->labelFundoAnime00Titulo->height()));
  ui->labelFundoAnime00Titulo->move(
      static_cast<int>(
          (static_cast<double>(ui->labelFundoAnime00Titulo->pos().rx()) /
           1920) *
          width),
      static_cast<int>(
          (static_cast<double>(ui->labelFundoAnime00Titulo->pos().ry()) / 821) *
          widheight));
  ui->labelAnime00Titulo->resize(
      static_cast<int>(comprimentoJanela * ui->labelAnime00Titulo->width()),
      static_cast<int>(alturaJanela * ui->labelAnime00Titulo->height()));
  ui->labelAnime00Titulo->move(
      static_cast<int>(
          (static_cast<double>(ui->labelAnime00Titulo->pos().rx()) / 1920) *
          width),
      static_cast<int>(
          (static_cast<double>(ui->labelAnime00Titulo->pos().ry()) / 821) *
          widheight));
  ui->imagemAnimeGrande->resize(
      static_cast<int>(comprimentoJanela * ui->imagemAnimeGrande->width()),
      static_cast<int>(alturaJanela * ui->imagemAnimeGrande->height()));
  ui->imagemAnimeGrande->move(
      static_cast<int>(
          (static_cast<double>(ui->imagemAnimeGrande->pos().rx()) / 1920) *
          width),
      static_cast<int>(
          (static_cast<double>(ui->imagemAnimeGrande->pos().ry()) / 821) *
          widheight));

  // Começo a aplicar isso aos outros labels. Pra isso, preciso fazer uma lista
  // com todos os labels disponíveis
  ...
  foreach (QObject *wid, widgetList) {
    ...
    // No caso, se eles forem labeis da mesma categoria que o que escolhi pra
    // referência, eles devem ter sempre o mesmo tamanho Mas diferentes posições
    if ((thisWid->objectName().contains("botaoAnime", Qt::CaseInsensitive) ||
      ...
      this->findChild<QWidget *>(wid->objectName())
          ->resize(static_cast<int>(comprimentoJanela * larguraLabelAnime),
                   static_cast<int>(alturaJanela * alturaLabelAnime));
      QString vposicaoGrade;
      if (thisWid->objectName().contains("botaoAnime", Qt::CaseInsensitive))
        vposicaoGrade = thisWid->objectName().remove("botaoAnime");
      else if (thisWid->objectName().contains("imagemAnime",
                                              Qt::CaseInsensitive))
      ...
      if (vposicaoGrade.toInt() % 3 == 0)
        thisWid->move(ui->imagemAnime00->x(),
                      static_cast<int>(ui->imagemAnime00->y() +
                                       ((ui->imagemAnime00->height() + 2) *
        ...
        thisWid->move((ui->labelFundoAnime00Titulo->x() +
                      ...
                      static_cast<int>(ui->imagemAnime00->y() +
                                       ((ui->imagemAnime00->height() + 2) *
                                        (vposicaoGrade.toInt() - 2) / 3)));
    }
    // Labelfundo e label anime são separados por motivos de tamanho do label
    else if (thisWid->objectName().contains("labelFundoAnime",
                                            Qt::CaseInsensitive)) {
    ...
    // Agora começa a mudar os textos
    else if (thisWid->objectName().contains("labelAnime",
                                            Qt::CaseInsensitive) &&
             !thisWid->objectName().contains("assistindo",
                                             Qt::CaseInsensitive)) {
      if (thisWid->objectName().contains("titulo", Qt::CaseInsensitive))
        thisWid->resize(ui->labelAnime00Titulo->width(),
      ...
      if (thisWid->objectName().contains("titulo", Qt::CaseInsensitive)) {
        ...
        vposicaoGrade = vposicaoGrade.remove("progresso", Qt::CaseInsensitive);
        ypos =
            ui->labelAnime00Titulo->y() + ui->labelAnime00Titulo->height() + 2;
      } else if (thisWid->objectName().contains("nota", Qt::CaseInsensitive)) {
        vposicaoGrade = vposicaoGrade.remove("nota", Qt::CaseInsensitive);
        ypos = static_cast<int>(ui->labelAnime00Titulo->y() +
                                (ui->labelAnime00Titulo->height() * 1.5) + 4);
      } else if (thisWid->objectName().contains("lista", Qt::CaseInsensitive)) {
        vposicaoGrade = vposicaoGrade.remove("lista", Qt::CaseInsensitive);
      ...
      if (vposicaoGrade.toInt() % 3 == 0) {
        thisWid->move(
            ui->labelAnime00Titulo->x(),
            static_cast<int>(ypos + ((2 + ui->imagemAnime00->height()) *
                                     (vposicaoGrade.toInt() / 3))));
      } else if ((vposicaoGrade.toInt() - 1) % 3 == 0)
        thisWid->move(
            ui->labelAnime00Titulo->x() + ui->labelAnime00Titulo->width() + 13 +
                ui->imagemAnime00->width(),
            static_cast<int>(ypos + ((2 + ui->imagemAnime00->height()) *
                                     (vposicaoGrade.toInt() / 3))));
      else if ((vposicaoGrade.toInt() - 2) % 3 == 0)
        thisWid->move(
            ui->labelAnime00Titulo->x() + ((ui->labelAnime00Titulo->width() +
                                            13 + ui->imagemAnime00->width()) *
                                           2),
            static_cast<int>(ypos + ((2 + ui->imagemAnime00->height()) *
                                     (vposicaoGrade.toInt() / 3))));
    }
    ...
    //        else if(this->findChild<QWidget
    //        *>(wid->objectName())->objectName().contains("labelInfo",
    //        Qt::CaseInsensitive)){
    else if (!thisWid->objectName().contains("imagemanimegrande",
                                             Qt::CaseInsensitive)) {
      thisWid->resize(static_cast<int>(comprimentoJanela * thisWid->width()),
                      static_cast<int>(alturaJanela * thisWid->height()));
      thisWid->move(
          static_cast<int>((static_cast<double>(thisWid->x()) / 1920) * width),
          static_cast<int>((static_cast<double>(thisWid->y()) / 821) *
                           widheight));
      if (thisWid->objectName().contains("labelDireitoFundoControles",
                                         Qt::CaseInsensitive) &&
          (thisWid->y() == ui->labelDireitoFundoSinopse->y() +
                               ui->labelDireitoFundoSinopse->height())) {
  ...
  if (width <= 1366) {
    vtituloNormalMedio = "background: transparent; font: 75 5pt \"Calibri\"; "
                         "font-weight: bold; color: rgb(20, 20, 20);";
    vtituloNormalPequeno = "background: transparent; font: 75 4pt \"Calibri\"; "
                           "font-weight: bold; color: rgb(20, 20, 20);";
    vfonteNormal = "background: transparent; font: 75 6pt \"Calibri\"; "
                   "font-weight: bold; color: rgb(20, 20, 20);";
    vfonteNormalNota = "background: transparent; font: 75 6pt \"Calibri\"; "
                       "font-weight: bold; color: rgb(53, 68, 120);";
    vfonteNormalAnimeEncontrado =
        "background: transparent; font: 75 6pt \"Calibri\"; font-weight: bold; "
        "color: rgb(53, 120, 56);";
    vfonteNormalAnimeNaoComecou =
        "background: transparent; font: 75 6pt \"Calibri\"; font-weight: bold; "
        "color: rgb(120, 60, 53);";
...
void MainWindow::on_botaoAnimeAssistindo_clicked() {
  ui->barraBusca->setPlainText(
      cleitorListaAnimes->fbuscaAnimePorIDERetornaTitulo(idAnimeAssistindo));
  on_botaoBusca_clicked();
  ui->barraBusca->clear();
}
...
void MainWindow::on_botaoSelecionarPastaAnime_clicked() {
  ...
  if (dir != "") {
    cconfUsuario->fselecionaPastaEspecificaAnime(
        vlistaSelecionada[vanimeSelecionado]->vid, dir);
  }
}
...
void MainWindow::on_botaoSeason_clicked() {
  ...
  if (vlistaSelecionada.isEmpty())
    ...
    ui->labelMensagem->setText("The images from animes that are not in some "
                               "list are not downloaded by default.\n"
                               "If needed, you can use the Load button to "
                               "download and load the images from the "
                               "animes in this year.");
  ui->NumPagina->setText(
...
void MainWindow::on_botaoAddAlternativeTitle_clicked() {
  if (!ui->barraAddNomeAlternativo->toPlainText().isEmpty()) {
    ...
    if (!vcustomNomesAlternativos.contains(
            vlistaSelecionada[vanimeSelecionado]->vid))
      vcustomNomesAlternativos.insert(vlistaSelecionada[vanimeSelecionado]->vid,
                                      tempAnimeList);
    else
      vcustomNomesAlternativos[vlistaSelecionada[vanimeSelecionado]->vid]
          .append((ui->barraAddNomeAlternativo->toPlainText()));
    cleitorListaAnimes->finsereNomeAlternativo(
        vlistaSelecionada[vanimeSelecionado]->vid, tempAnimeList);
...
void MainWindow::on_botaoDownloadNovaImagem_clicked() {
  ...
  mediumFile->setFileName(dirMedio + vlistaSelecionada[vanimeSelecionado]->vid +
                          ".jpg");
  bigFile->setFileName(dirGrande + vlistaSelecionada[vanimeSelecionado]->vid +
                       ".jpg");
