...
bool arquivos::fcomparaDadosAnime(QString rfileName, const QString &rnomeAnime,
                                  QString rnomeAnimeIngles,
                                  const QStringList &rnomesAlternativosAnime,
  ...
  // Usamos isso para pegar o número do episódio e o nome do anime a partir do
  // nome do arquivo
  rfileName =
      QString::fromStdWString(lelements.get(anitomy::kElementAnimeTitle));
  QString vtemporada =
      QString::fromStdWString(lelements.get(anitomy::kElementAnimeSeason));
  ...
  if (formatador.fcomparaNomes(rfileName, rnomeAnime) &&
      (lepisodioAnime == repisodioAnime + 1 ||
       lepisodioAnime - repisodiosTotais == repisodioAnime + 1)) {
    return true;
  } else if (formatador.fcomparaNomes(rfileName, std::move(rnomeAnimeIngles)) &&
             (lepisodioAnime == repisodioAnime + 1 ||
              lepisodioAnime - repisodiosTotais == repisodioAnime + 1)) {
    return true;
  } else {
    for (int i = 0; i < rnomesAlternativosAnime.size(); i++) {
      if (formatador.fcomparaNomes(rfileName, rnomesAlternativosAnime.at(i)) &&
          (lepisodioAnime == repisodioAnime + 1 ||
           lepisodioAnime - repisodiosTotais == repisodioAnime + 1)) {
        return true;
      }
...
int arquivos::fcomparaSeasons(QString rnome, int repisodio, int rtemporada) {
  ...
  // Se forem animes, checo se a temporada é diferente da que eu busco, e caso
  // for menor, eu só somo todos os episódios Pra obter o número total de
  // episódios
  ...
  for (int i = 0; i < vlistaSelecionada.size(); i++) {
    ...
    if (rnome.compare(nomeAnimeTemp) == 0 &&
