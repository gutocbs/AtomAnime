...
void filedownloader::fsetLeitorListaAnimes(
    leitorlistaanimes *lleitorlistaanimes) {
...
void filedownloader::onReadyRead() {
  if (vfile->isWritable()) {
    ...
    // qWarning() << vfile->errorString();// << "- Id:" <<
    // vlistaSelecionada[vindexLista]->vid << "- Medium image download failed";
    vfile->remove();
  }
}
...
void filedownloader::fdownloadAvatarUsuario(QString fileURL) {
  ...
  connect(vmanager, SIGNAL(finished(QNetworkReply *)), this,
...
void filedownloader::fdownloadMedio() {
  ...
  if (!vlistaSelecionada.isEmpty()) {
    ...
    if (!(QFileInfo::exists(vsaveFilePath) &&
          QFileInfo(vsaveFilePath).isFile())) {
      QNetworkRequest lrequest;
...
void filedownloader::fdownloadGrande() {
  ...
  if (!vlistaSelecionada.isEmpty()) {
    ...
    if (QFile(vsaveFilePath).exists() && QFile(vsaveFilePath).size() == 0)
      QFile(vsaveFilePath).remove();
    //    qDebug() << vsaveFilePath << vlistaSelecionada[vindexLista]->vid <<
    //    vlistaSelecionada[vindexLista]->vLinkImagemMedia.mid(
...
void filedownloader::fdownloadPequeno() {
  ...
  vsaveFilePath.append(vlistaSelecionada[vindexLista]->vLinkImagemMedia.mid(
      vlistaSelecionada[vindexLista]->vLinkImagemMedia.lastIndexOf(
          QChar('.'))));
  ...
  bool fileExists =
      QFileInfo::exists(vsaveFilePath) && QFileInfo(vsaveFilePath).isFile();
