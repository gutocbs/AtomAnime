...
void ChecaInfoPlayer::fchecaStream(const QString &player,
                                   const QString &janela) {
  fgetStreamLanguages();
  if (!janela.isEmpty()) {
...
QString ChecaInfoPlayer::fretornaAnime() { return vanime; }

QString ChecaInfoPlayer::fretornaEpisodio() { return vepisodio; }
...
void ChecaInfoPlayer::fchecaCrunchyroll() {
  ...
  if (!vStreamLanguages.isEmpty()) {
    for (int i = 1; i < vStreamLanguages.size(); i++) {
      if (nomeJanela.contains(vStreamLanguages.at(i)) &&
          vStreamLanguages.at(i).compare("الحلقة") != 0) {
        indexOfEpisode = nomeJanela.indexOf(vStreamLanguages.at(i));
        vepisodio = nomeJanela.at(indexOfEpisode + 2);
        break;
      } else if (nomeJanela.contains(vStreamLanguages.at(i)) &&
                 vStreamLanguages.at(i).compare("الحلقة") == 0) {
        indexOfEpisode = nomeJanela.indexOf(vStreamLanguages.at(i));
        vepisodio = nomeJanela.at(indexOfEpisode - 2);
        break;
