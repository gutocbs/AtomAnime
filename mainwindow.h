...
private slots:
  ...
  void fMostraAnimeAberto(QString, const QString &rnomeAnime,
                          const QString &repisodioAnime);
  void fAumentaProgressoID(const QString &ridAnime,
                           const QString &episodioAnime);
...
private:
  ...
  // A classe já está deletando todos os ponteiros. Tentar deletar isso vai ser
  // tentar deletar uma lista vaiza e possivelemnte irá Crashar o programa
  ...
  // Thread que vai carregar as imagens do anime. Como essa thread fica dentro
  // da própria classe, temos que usar QFuture
  ...
  QMutex vmutex;
  // Thread que vai baixar a lista de anime e busca as pastas. Como essa thread
  // fica fora da classe, temos que usar QThread
  QThread cThread;
  QThread dThread;
