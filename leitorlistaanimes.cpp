...
leitorlistaanimes::~leitorlistaanimes() {
  //    fdeletaListaAnimes();
  qDeleteAll(vlistaWatching.begin(),
             vlistaWatching.end()); // Deletar todas as listas
  qDeleteAll(vlistaCompleted.begin(),
             vlistaCompleted.end()); // Deletar todas as listas
  qDeleteAll(vlistaDropped.begin(),
             vlistaDropped.end()); // Deletar todas as listas
  qDeleteAll(vlistaOnHold.begin(),
             vlistaOnHold.end()); // Deletar todas as listas
  qDeleteAll(vlistaPlanToWatch.begin(),
             vlistaPlanToWatch.end());                // Deletar todas as listas
...
bool leitorlistaanimes::fleJson() {
  ...
  QString lnomeIngles;
  QStringList
      lnomeAlternativo; // StringList por que pode ter lários nomes alternativos
      ...
      if (llinha.contains("\"large\":")) {
        ...
        if (llinha.contains("crunchyroll")) {
          ...
          if (llinha.contains("?"))
            llinha.replace("?", "null");
          lstreamCrunchyroll =
              llinha.left(llinha.lastIndexOf("episode-")).trimmed();
        }
        ...
        if (llinha == "FINISHED")
          ...
          ldataEpisodioFinal =
              QLocale(QLocale::English)
                  .toString(ldataEpisodioConvertida, "MMMM d ");
          ldataEpisodioFinal.append(lhoraLancamentoEpisodio.toString("hh:mm"));
        ...
        tempNomeAnime.clear();
        if (lformato != "MANGA" && lformato != "NOVEL" &&
            lformato != "ONE SHOT") {
          if (llinha.trimmed() == "CURRENT") {
            ...
            if (!vHashPosicaoAnimesPorId.contains(lnovoAnime->vid))
              vHashPosicaoAnimesPorId.insert(lnovoAnime->vid,
                                             vlistaWatching.size() - 1);
            else if (vHashPosicaoAnimesPorId[lnovoAnime->vid] !=
                     vlistaWatching.size() - 1)
              vHashPosicaoAnimesPorId.insert(lnovoAnime->vid,
                                             vlistaWatching.size() - 1);
          ...
          if (llinha.trimmed() == "CURRENT") {
            ...
            if (!vHashPosicaoMangasPorId.contains(lnovoAnime->vid))
              vHashPosicaoMangasPorId.insert(lnovoAnime->vid,
                                             vlistaMangaReading.size() - 1);
            else if (vHashPosicaoMangasPorId[lnovoAnime->vid] !=
                     vlistaMangaReading.size() - 1)
              vHashPosicaoMangasPorId.insert(lnovoAnime->vid,
                                             vlistaMangaReading.size() - 1);
          ...
          if (llinha.trimmed() == "CURRENT") {
            ...
            if (!vHashPosicaoNovelsPorId.contains(lnovoAnime->vid))
              vHashPosicaoNovelsPorId.insert(lnovoAnime->vid,
                                             vlistaNovelReading.size() - 1);
            else if (vHashPosicaoNovelsPorId[lnovoAnime->vid] !=
                     vlistaNovelReading.size() - 1)
              vHashPosicaoNovelsPorId.insert(lnovoAnime->vid,
                                             vlistaNovelReading.size() - 1);
...
QVector<anime *> leitorlistaanimes::fleListaAno(int ano) {
  ...
  QFile lleJson("Configurações/Temp/Lists/animeList" + QString::number(ano) +
                ".txt");
  if (lleJson.size() == 0)
  ...
  if (lleJson.open(QIODevice::ReadOnly)) {
    while (!json.atEnd()) {
      ...
      if (llinha.contains("\"large\":")) {
        ...
        lnovoAnime->vnumEpisodiosAssistidos =
            fbuscaAnimePorIDERetornaEpisodio(lid);
...
QVector<anime *>
leitorlistaanimes::fleListaAnoSeason(int ano, const QString &seasonEscolhida) {
  QVector<anime *> vetorTemporario;
  QFile lleJson("Configurações/Temp/Lists/animeList" + QString::number(ano) +
                ".txt");
  if (lleJson.size() == 0)
      ...
      if (encontrou) {
        if (llinha.contains("\"url\":")) {
          ...
          if (fbuscaAnimePorIDERetornaLista(lid).isEmpty())
            lnovoAnime->vlista =
                fbuscaAnimePorIDERetornaLista(lid) + " " + lseason;
          else
            lnovoAnime->vlista = fbuscaAnimePorIDERetornaLista(lid);
          lnovoAnime->vnumEpisodiosAssistidos =
              fbuscaAnimePorIDERetornaEpisodio(lid);
...
void leitorlistaanimes::fsalvaListaIdsNomesAnos() {
  ...
  if (larquivo.open(QIODevice::WriteOnly)) {
    ...
    foreach (QString key, vHashNomeAnimesPorId.keys()) {
      lstreamTexto << key << ";" << vHashListaAnimesPorId[key] << ";"
                   << vHashPosicaoAnimesPorId[key] << ";"
  ...
  if (larquivoManga.open(QIODevice::WriteOnly)) {
    ...
    foreach (QString key, vHashNomeMangasPorId.keys()) {
      lstreamTexto << key << ";" << vHashListaMangasPorId[key] << ";"
                   << vHashPosicaoMangasPorId[key] << ";"
  ...
  if (larquivoNovel.open(QIODevice::WriteOnly)) {
    ...
    foreach (QString key, vHashNomeNovelsPorId.keys()) {
      lstreamTexto << key << ";" << vHashListaNovelsPorId[key] << ";"
                   << vHashPosicaoNovelsPorId[key] << ";"
  ...
  if (larquivoSizePorAno.open(QIODevice::WriteOnly)) {
    ...
    foreach (int key, vHashSizeListasPorAno.keys()) {
      lstreamTexto << QString::number(key) << ";" << vHashSizeListasPorAno[key]
                   << endl;
    }
    larquivoSizePorAno.close();
  }
...
void leitorlistaanimes::fcarregaListaAnoEmThread() {
  // Essa função serve para saber se os animes do ano x já estão salvos na lista
  // de ids. Quando eles já estão salvos, o ano vai ser salvo no hash. Isso é
  // importante para a função de download de torrents, onde eles vão buscar o id
  // dos animes.
...
void leitorlistaanimes::fdeletaListaAnimes() {
  ...
  //        qDeleteAll(vlistaBusca.begin(),vlistaBusca.end()); //Deletar todas
  //        as listas vlistaBusca.clear();
  //    }
  if (!vlistaMangaReading.isEmpty()) {
    qDeleteAll(vlistaMangaReading.begin(),
               vlistaMangaReading.end()); // Deletar todas as listas
    vlistaMangaReading.clear();
  }
  if (!vlistaMangaCompleted.isEmpty()) {
    qDeleteAll(vlistaMangaCompleted.begin(),
               vlistaMangaCompleted.end()); // Deletar todas as listas
    vlistaMangaCompleted.clear();
  }
  if (!vlistaMangaDropped.isEmpty()) {
    qDeleteAll(vlistaMangaDropped.begin(),
               vlistaMangaDropped.end()); // Deletar todas as listas
    vlistaMangaDropped.clear();
  }
  if (!vlistaMangaOnHold.isEmpty()) {
    qDeleteAll(vlistaMangaOnHold.begin(),
               vlistaMangaOnHold.end()); // Deletar todas as listas
    vlistaMangaOnHold.clear();
  }
  if (!vlistaMangaPlanToRead.isEmpty()) {
    qDeleteAll(vlistaMangaPlanToRead.begin(),
               vlistaMangaPlanToRead.end()); // Deletar todas as listas
    vlistaMangaPlanToRead.clear();
  }
  if (!vlistaNovelReading.isEmpty()) {
    qDeleteAll(vlistaNovelReading.begin(),
               vlistaNovelReading.end()); // Deletar todas as listas
    vlistaNovelReading.clear();
  }
  if (!vlistaNovelCompleted.isEmpty()) {
    qDeleteAll(vlistaNovelCompleted.begin(),
               vlistaNovelCompleted.end()); // Deletar todas as listas
    vlistaNovelCompleted.clear();
  }
  if (!vlistaNovelDropped.isEmpty()) {
    qDeleteAll(vlistaNovelDropped.begin(),
               vlistaNovelDropped.end()); // Deletar todas as listas
    vlistaNovelDropped.clear();
  }
  if (!vlistaNovelOnHold.isEmpty()) {
    qDeleteAll(vlistaNovelOnHold.begin(),
               vlistaNovelOnHold.end()); // Deletar todas as listas
    vlistaNovelOnHold.clear();
  }
  if (!vlistaNovelPlanToRead.isEmpty()) {
    qDeleteAll(vlistaNovelPlanToRead.begin(),
               vlistaNovelPlanToRead.end()); // Deletar todas as listas
    vlistaNovelPlanToRead.clear();
  }
}

QString leitorlistaanimes::fprocuraAnimeNasListas(const QString &rnomeAnime) {
  // Essa função é importante por que se ela não encontrar o nome certo logo de
  // cara, vai fazer uma busca mais detalhada e adicionar o nome procurado nas
  // informações do anime, quando encontrar.
  foreach (QString key, vHashNomeAnimesPorId.keys()) {
  ...
  for (int i = 0; i < vlistaWatching.size(); i++) {
    if (formatador.fcomparaNomes(vlistaWatching[i]->vnome, rnomeAnime) ||
      ...
      for (int w = 0; w < vlistaWatching[i]->vnomeAlternativo.size(); w++) {
        if (formatador.fcomparaNomes(vlistaWatching[i]->vnomeAlternativo.at(w),
                                     rnomeAnime)) {
          tempNomeAnime.append(rnomeAnime);
          if (!vHashNomeAnimesPorId.contains(vlistaWatching[i]->vid))
            vHashNomeAnimesPorId.insert(vlistaWatching[i]->vid, tempNomeAnime);
  ...
  for (int i = 0; i < vlistaPlanToWatch.size(); i++) {
    if (formatador.fcomparaNomes(vlistaPlanToWatch[i]->vnome, rnomeAnime) ||
        formatador.fcomparaNomes(vlistaPlanToWatch[i]->vnomeIngles,
                                 rnomeAnime)) {
      tempNomeAnime.append(rnomeAnime);
      ...
      for (int w = 0; w < vlistaPlanToWatch[i]->vnomeAlternativo.size(); w++) {
        if (formatador.fcomparaNomes(
                vlistaPlanToWatch[i]->vnomeAlternativo.at(w), rnomeAnime)) {
          tempNomeAnime.append(rnomeAnime);
          if (!vHashNomeAnimesPorId.contains(vlistaPlanToWatch[i]->vid))
            vHashNomeAnimesPorId.insert(vlistaPlanToWatch[i]->vid,
                                        tempNomeAnime);
          else
            vHashNomeAnimesPorId[vlistaPlanToWatch[i]->vid].append(
                tempNomeAnime);
          return vlistaPlanToWatch[i]->vid;
        }
      }
  ...
  for (int i = 0; i < vlistaOnHold.size(); i++) {
    if (formatador.fcomparaNomes(vlistaOnHold[i]->vnome, rnomeAnime) ||
      ...
      for (int w = 0; w < vlistaOnHold[i]->vnomeAlternativo.size(); w++) {
        if (formatador.fcomparaNomes(vlistaOnHold[i]->vnomeAlternativo.at(w),
                                     rnomeAnime)) {
          tempNomeAnime.append(rnomeAnime);
          if (!vHashNomeAnimesPorId.contains(vlistaOnHold[i]->vid))
            vHashNomeAnimesPorId.insert(vlistaOnHold[i]->vid, tempNomeAnime);
  ...
  for (int i = 0; i < vlistaDropped.size(); i++) {
    if (formatador.fcomparaNomes(vlistaDropped[i]->vnome, rnomeAnime) ||
      ...
      for (int w = 0; w < vlistaDropped[i]->vnomeAlternativo.size(); w++) {
        if (formatador.fcomparaNomes(vlistaDropped[i]->vnomeAlternativo.at(w),
                                     rnomeAnime)) {
          tempNomeAnime.append(rnomeAnime);
          if (!vHashNomeAnimesPorId.contains(vlistaDropped[i]->vid))
            vHashNomeAnimesPorId.insert(vlistaDropped[i]->vid, tempNomeAnime);
  ...
  for (int i = 0; i < vlistaCompleted.size(); i++) {
    if (formatador.fcomparaNomes(vlistaCompleted[i]->vnome, rnomeAnime) ||
      ...
      for (int w = 0; w < vlistaCompleted[i]->vnomeAlternativo.size(); w++) {
        if (formatador.fcomparaNomes(vlistaCompleted[i]->vnomeAlternativo.at(w),
                                     rnomeAnime)) {
          tempNomeAnime.append(rnomeAnime);
          if (!vHashNomeAnimesPorId.contains(vlistaCompleted[i]->vid))
            vHashNomeAnimesPorId.insert(vlistaCompleted[i]->vid, tempNomeAnime);
...
QString
leitorlistaanimes::fprocuraIdNasListasRapido(const QString &rnomeAnime) {
  foreach (QString key, vHashNomeAnimesPorId.keys()) {
    for (int i = 0; i < vHashNomeAnimesPorId[key].size(); i++) {
      if (vHashNomeAnimesPorId[key].at(i) == rnomeAnime)
        return key;
...
void leitorlistaanimes::finsereNomeAlternativo(
    const QString &rid, const QStringList &rlistaNomesAlternativos) {
  ...
  if (llista.compare("Watching", Qt::CaseInsensitive) == 0) {
    ...
    vlistaPlanToWatch[lposicao]->vnomeAlternativo.append(
        rlistaNomesAlternativos);
    vHashNomeAnimesPorId[rid].append(rlistaNomesAlternativos);
...
bool leitorlistaanimes::fdeletedaLista(const QString &rid) {
  ...
  if (llista == "Watching") {
    vHashListaAnimesPorId.insert(
        rid, QString::number(vlistaWatching[lposicao]->vdataEstreia.year()));
    vHashPosicaoAnimesPorId.insert(rid, 0);
    vlistaWatching.remove(lposicao);
    return true;
  } else if (llista == "Completed") {
    vHashListaAnimesPorId.insert(
        rid, QString::number(vlistaCompleted[lposicao]->vdataEstreia.year()));
    vHashPosicaoAnimesPorId.insert(rid, 0);
    vlistaCompleted.remove(lposicao);
    return true;
  } else if (llista == "On Hold") {
    vHashListaAnimesPorId.insert(
        rid, QString::number(vlistaOnHold[lposicao]->vdataEstreia.year()));
    vHashPosicaoAnimesPorId.insert(rid, 0);
    vlistaOnHold.remove(lposicao);
    return true;
  } else if (llista == "Dropped") {
    vHashListaAnimesPorId.insert(
        rid, QString::number(vlistaDropped[lposicao]->vdataEstreia.year()));
    vHashPosicaoAnimesPorId.insert(rid, 0);
    vlistaDropped.remove(lposicao);
    return true;
  } else if (llista == "Plan to Watch") {
    vHashListaAnimesPorId.insert(
        rid, QString::number(vlistaPlanToWatch[lposicao]->vdataEstreia.year()));
    vHashPosicaoAnimesPorId.insert(rid, 0);
    vlistaPlanToWatch.remove(lposicao);
    return true;
  } else if (llista == "Reading(Manga)") {
    vHashListaMangasPorId.insert(
        rid,
        QString::number(vlistaMangaReading[lposicao]->vdataEstreia.year()));
    ...
    vHashListaMangasPorId.insert(
        rid,
        QString::number(vlistaMangaCompleted[lposicao]->vdataEstreia.year()));
    vHashPosicaoMangasPorId.insert(rid, 0);
    vlistaMangaCompleted.remove(lposicao);
    return true;
  } else if (llista == "On Hold(Manga)") {
    vHashListaMangasPorId.insert(
        rid, QString::number(vlistaMangaOnHold[lposicao]->vdataEstreia.year()));
    vHashPosicaoMangasPorId.insert(rid, 0);
    ...
    vHashListaMangasPorId.insert(
        rid,
        QString::number(vlistaMangaDropped[lposicao]->vdataEstreia.year()));
    vHashPosicaoMangasPorId.insert(rid, 0);
    ...
    vHashListaMangasPorId.insert(
        rid,
        QString::number(vlistaMangaPlanToRead[lposicao]->vdataEstreia.year()));
    vHashPosicaoMangasPorId.insert(rid, 0);
    vlistaMangaPlanToRead.remove(lposicao);
    return true;
  } else if (llista == "Reading(Novel)") {
    vHashListaNovelsPorId.insert(
        rid,
        QString::number(vlistaNovelReading[lposicao]->vdataEstreia.year()));
    ...
    vHashListaNovelsPorId.insert(
        rid,
        QString::number(vlistaNovelCompleted[lposicao]->vdataEstreia.year()));
    vHashPosicaoNovelsPorId.insert(rid, 0);
    vlistaNovelCompleted.remove(lposicao);
    return true;
  } else if (llista == "On Hold(Novel)") {
    vHashListaNovelsPorId.insert(
        rid, QString::number(vlistaNovelOnHold[lposicao]->vdataEstreia.year()));
    vHashPosicaoNovelsPorId.insert(rid, 0);
    ...
    vHashListaNovelsPorId.insert(
        rid,
        QString::number(vlistaNovelDropped[lposicao]->vdataEstreia.year()));
    vHashPosicaoNovelsPorId.insert(rid, 0);
    ...
    vHashListaNovelsPorId.insert(
        rid,
        QString::number(vlistaNovelPlanToRead[lposicao]->vdataEstreia.year()));
    vHashPosicaoNovelsPorId.insert(rid, 0);
...
QVector<anime *> leitorlistaanimes::sortLista(const QString &rordem,
                                              QString rlista) {
  if (rlista == "watching")
  ...
  if (!rordem.isEmpty()) {
    if (rordem.startsWith("c")) {
      if (rordem.contains("nome", Qt::CaseInsensitive)) {
        std::sort(
            llistaTemp.begin(), llistaTemp.end(),
            [](anime *a, anime *b) -> bool { return a->vnome < b->vnome; });
      } else if (rordem.contains("data", Qt::CaseInsensitive)) {
        std::sort(llistaTemp.begin(), llistaTemp.end(),
                  [](anime *a, anime *b) -> bool {
                    return a->vdataEstreia < b->vdataEstreia;
                  });
      } else if (rordem.contains("progresso", Qt::CaseInsensitive)) {
        std::sort(llistaTemp.begin(), llistaTemp.end(),
                  [](anime *a, anime *b) -> bool {
                    return a->vnumEpisodiosAssistidos.toInt() <
      ...
      if (rordem.contains("nome", Qt::CaseInsensitive)) {
        ...
        std::sort(
            llistaTemp.begin(), llistaTemp.end(),
            [](anime *a, anime *b) -> bool {
              return static_cast<float>(a->vnumEpisodiosAssistidos.toInt()) >
...
QVector<anime *> leitorlistaanimes::fbuscaLista(const QString &rnome,
                                                const QString &rtipoMidia) {
  vlistaBusca.clear();
  if (rtipoMidia.compare("anime", Qt::CaseInsensitive) == 0) {
    ...
    foreach (QString key, vHashNomeMangasPorId.keys()) {
      for (int i = 0; i < vHashNomeMangasPorId[key].size(); i++) {
        if (vHashNomeMangasPorId[key].at(i).contains(rnome,
                                                     Qt::CaseInsensitive)) {
          QString lista = vHashListaMangasPorId[key];
          int pos = vHashPosicaoMangasPorId[key];
          if (pos == -1)
            break;
          else if (lista.compare("Reading(Manga)", Qt::CaseInsensitive) == 0 &&
                   vlistaMangaReading.size() > pos) {
            vlistaBusca.append(vlistaMangaReading[pos]);
            break;
          } else if (lista.compare("Plan to Read(Manga)",
                                   Qt::CaseInsensitive) == 0 &&
                     vlistaMangaPlanToRead.size() > pos) {
            vlistaBusca.append(vlistaMangaPlanToRead[pos]);
            break;
          } else if (lista.compare("On Hold(Manga)", Qt::CaseInsensitive) ==
                         0 &&
                     vlistaMangaOnHold.size() > pos) {
            vlistaBusca.append(vlistaMangaOnHold[pos]);
            break;
          } else if (lista.compare("Completed(Manga)", Qt::CaseInsensitive) ==
                         0 &&
                     vlistaMangaCompleted.size() > pos) {
            vlistaBusca.append(vlistaMangaCompleted[pos]);
            break;
          } else if (lista.compare("Dropped(Manga)", Qt::CaseInsensitive) ==
                         0 &&
                     vlistaMangaDropped.size() > pos) {
    ...
    foreach (QString key, vHashNomeNovelsPorId.keys()) {
      for (int i = 0; i < vHashNomeNovelsPorId[key].size(); i++) {
        if (vHashNomeNovelsPorId[key].at(i).contains(rnome,
                                                     Qt::CaseInsensitive)) {
          QString lista = vHashListaNovelsPorId[key];
          int pos = vHashPosicaoNovelsPorId[key];
          if (pos == -1)
            break;
          else if (lista.compare("Reading(Novel)", Qt::CaseInsensitive) == 0 &&
                   vlistaNovelReading.size() > pos) {
            vlistaBusca.append(vlistaNovelReading[pos]);
            break;
          } else if (lista.compare("Plan to Read(Novel)",
                                   Qt::CaseInsensitive) == 0 &&
                     vlistaNovelPlanToRead.size() > pos) {
            vlistaBusca.append(vlistaNovelPlanToRead[pos]);
            break;
          } else if (lista.compare("On Hold(Novel)", Qt::CaseInsensitive) ==
                         0 &&
                     vlistaNovelOnHold.size() > pos) {
            vlistaBusca.append(vlistaNovelOnHold[pos]);
            break;
          } else if (lista.compare("Completed(Novel)", Qt::CaseInsensitive) ==
                         0 &&
                     vlistaNovelCompleted.size() > pos) {
            vlistaBusca.append(vlistaNovelCompleted[pos]);
            break;
          } else if (lista.compare("Dropped(Novel)", Qt::CaseInsensitive) ==
                         0 &&
                     vlistaNovelDropped.size() > pos) {
...
QString
leitorlistaanimes::fbuscaAnimePorIDERetornaEpisodio(const QString &rid) {
  QString llista = fbuscaAnimePorIDERetornaLista(rid);
  int lposicao = fbuscaAnimePorIDERetornaPosicao(rid);
  if (lposicao == -1)
    return "0";
...
QString
leitorlistaanimes::fbuscaAnimePorIDERetornaLista(const QString &ridAnime) {
...
int leitorlistaanimes::fbuscaAnimePorIDERetornaPosicao(
    const QString &ridAnime) {
  ...
  if (vHashPosicaoAnimesPorId.contains(ridAnime)) {
      ...
      for (int i = 0; i < vlistaWatching.size(); i++) {
        if (vlistaWatching[i]->vid.compare(ridAnime) == 0) {
          ...
          return i;
               ...
               (vlistaPlanToWatch.size() <= lposicao ||
    ...
    if (llista.compare("Reading(Manga)", Qt::CaseInsensitive) == 0 &&
        (vlistaMangaReading.size() <= lposicao ||
      ...
      for (int i = 0; i < vlistaMangaReading.size(); i++) {
        if (vlistaMangaReading[i]->vid.compare(ridAnime) == 0) {
          ...
          return i;
               ...
               (vlistaMangaCompleted.size() <= lposicao ||
      ...
      for (int i = 0; i < vlistaMangaCompleted.size(); i++) {
        if (vlistaMangaCompleted[i]->vid.compare(ridAnime) == 0) {
          ...
          return i;
               ...
               (vlistaMangaOnHold.size() <= lposicao ||
      ...
      for (int i = 0; i < vlistaMangaOnHold.size(); i++) {
        if (vlistaMangaOnHold[i]->vid.compare(ridAnime) == 0) {
          ...
          return i;
               ...
               (vlistaMangaDropped.size() <= lposicao ||
    ...
    } else if (llista.compare("Plan to Read(Manga)", Qt::CaseInsensitive) ==
                   0 &&
               (vlistaMangaPlanToRead.size() <= lposicao ||
    ...
    if (llista.compare("Reading(Novel)", Qt::CaseInsensitive) == 0 &&
        (vlistaNovelReading.size() <= lposicao ||
      ...
      for (int i = 0; i < vlistaNovelReading.size(); i++) {
        if (vlistaNovelReading[i]->vid.compare(ridAnime) == 0) {
          ...
          return i;
               ...
               (vlistaNovelCompleted.size() <= lposicao ||
      ...
      for (int i = 0; i < vlistaNovelCompleted.size(); i++) {
        if (vlistaNovelCompleted[i]->vid.compare(ridAnime) == 0) {
          ...
          return i;
               ...
               (vlistaNovelOnHold.size() <= lposicao ||
    ...
    } else if (llista.compare("Plan to Read(Novel)", Qt::CaseInsensitive) ==
                   0 &&
               (vlistaNovelDropped.size() <= lposicao ||
      ...
      for (int i = 0; i < vlistaNovelDropped.size(); i++) {
        if (vlistaNovelDropped[i]->vid.compare(ridAnime) == 0) {
          ...
          return i;
               ...
               (vlistaNovelPlanToRead.size() <= lposicao ||
...
bool leitorlistaanimes::fmudaProgresso(const QString &rid,
                                       const QString &rprogresso) {
  QString llista = fbuscaAnimePorIDERetornaLista(rid);
  int lposicao = fbuscaAnimePorIDERetornaPosicao(rid);
  if (lposicao == -1)
    return false;
...
bool leitorlistaanimes::fmudaLista(const QString &rid, const QString &rlista,
                                   const QString &rtipo) {
  QString llista = fbuscaAnimePorIDERetornaLista(rid);
  int lposicao = fbuscaAnimePorIDERetornaPosicao(rid);
  if (lposicao == -1)
