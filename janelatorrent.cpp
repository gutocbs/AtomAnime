  ...
  while (!stream.atEnd()) {
    ...
    case QXmlStreamReader::StartElement:
      if (stream.name() == "title") {
        ...
        for (int w = 0; w < cconfig->fretornaDownloadListasAnimes().size();
             w++) {
          ...
          for (int i = 0; i < vlistaAtual.size(); i++) {
            lnomeAnimeListaSimplicado =
                carquivos->fremoveCaracteresDiferentes(vlistaAtual[i]->vnome);
            if (lnomeAnimeSimplificado.compare(lnomeAnimeListaSimplicado,
                                               Qt::CaseInsensitive) == 0) {
              lprioridade++;
              if (lepisodioAnime.toInt() >
                  vlistaAtual[i]->vnumEpisodiosAssistidos.toInt()) {
                // Checa se é o próximo episódio que deve ser assistido. Se
                // estiver vazio, o episódio não foi baixado Se não estiver, o
                // episódio já existe no computador.
                QString lultimoEpisodioBaixado =
                    carquivos->fprocuraEpisodioEspecifico(
                        vlistaAtual[i], lepisodioAnime.toInt());
        ...
        if (lfansub.compare(cconfig->fretornaSubEscolhido(),
                            Qt::CaseInsensitive) == 0) {
          lprioridade++;
        }
        lresolucao = QString::fromStdWString(
            elements.get(anitomy::kElementVideoResolution));
        if (cconfig->fretornaQualidadeEscolhida().compare(
                lresolucao.toUtf8(), Qt::CaseInsensitive) == 0)
          lprioridade++;
      }
      if (stream.name() == "link") {
      ...
      if (stream.name() == "description") {
        ...
        if (vbaixar.contains(lnomeAnime + lepisodioAnime)) {
          if (vbaixar.value(lnomeAnime + lepisodioAnime).at(0).toInt() < 4 &&
              lprioridade == 4) {
...
void janelatorrent::on_botaoDownload_clicked() { ////CHECAR SE É WINDOWS OU
  ...
  if (cconfig->fretornaTorrentEscolhido() == "uTorrent")
    QDesktopServices::openUrl(QUrl("file:///" + QDir::homePath() +
                                       "/AppData/Roaming/uTorrent/uTorrent.exe",
                                   QUrl::TolerantMode));
  else if (cconfig->fretornaTorrentEscolhido() == "qBittorrent") {
    QDesktopServices::openUrl(
        QUrl("file:///C:/Program Files/qBittorrent/qbittorrent.exe",
             QUrl::TolerantMode));
  }
  for (int i = 0; i < torrent.size(); i++) {
...
void janelatorrent::fbaixaTorrent() {
  ...
  if (cconfig->fretornaTorrentEscolhido() == "uTorrent") {
    lprocesso.execute(
        QDir::homePath() + "/AppData/Roaming/uTorrent/uTorrent.exe",
        QStringList() << "/DIRECTORY"
                      << cconfig->fretornaPastaSalvarAnimes() + "/" +
                             torrent[vlistaDownload[0]]->vnomeAnime
                      << QDir::currentPath() + "/Configurações/Temp/Torrents/" +
                             torrent[vlistaDownload[0]]->vnomeTorrent +
                             ".torrent");
  } else if (cconfig->fretornaTorrentEscolhido() == "qBittorrent") {
    lprocesso.execute(
        "\"C:/Program Files/qBittorrent/qbittorrent.exe\" --add-paused=false "
        "--skip-dialog=true --save-path=" +
            cconfig->fretornaPastaSalvarAnimes() + "/" +
            torrent[vlistaDownload[0]]->vnomeAnime,
        QStringList() << QDir::currentPath() + "/Configurações/Temp/Torrents/" +
                             torrent[vlistaDownload[0]]->vnomeTorrent +
                             ".torrent");
  }
