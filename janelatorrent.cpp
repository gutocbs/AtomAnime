...
janelatorrent::janelatorrent(QWidget *parent)
    : QWidget(parent), ui(new Ui::janelatorrent) {
  ui->setupUi(this);
  ...
  //    int hhh = 1920 - ui->listaTorrents->columnWidth(1) -
  //    ui->listaTorrents->columnWidth(2) - ui->listaTorrents->columnWidth(3)
  //            - ui->listaTorrents->columnWidth(4) -
  //            ui->listaTorrents->columnWidth(5) -
  //            ui->listaTorrents->columnWidth(6);
  //    qDebug() << hhh;
  ui->listaTorrents->setSortingEnabled(true);
  cconfig = nullptr;
  cleitor = nullptr;
  carquivos = nullptr;
  connect(this, &janelatorrent::fimDownloadTorrent, this,
          &janelatorrent::fpreencheTabela, Qt::QueuedConnection);
}
...
// SEMPRE vou pegar animes não baixados, desde que estejam na lista e/ou cumpram
// os filtros. SE já tiver visto/baixado, ou tenha um filtro NOT, não entra na
// lista.
  ...
  while (!stream.atEnd()) {
    ...
    case QXmlStreamReader::StartElement:
      if (stream.name() == "title") {
        ...
        lepisodioAnime = QString::fromStdWString(
            elements.get(anitomy::kElementEpisodeNumber));
        lnomeAnime =
            QString::fromStdWString(elements.get(anitomy::kElementAnimeTitle));
        QString vtemporada =
            QString::fromStdWString(elements.get(anitomy::kElementAnimeSeason));
        if (!vtemporada.isEmpty())
          lnomeAnime.append(QString(" " + vtemporada));
        lfansub = QString::fromStdWString(
            elements.get(anitomy::kElementReleaseGroup));
        lresolucao = QString::fromStdWString(
            elements.get(anitomy::kElementVideoResolution));
        ...
        if (vHashDeNomeEId.contains(lnomeAnime))
          ...
          //                    lid =
          //                    cleitor->fprocuraAnimeNasListas(lnomeAnime);
        ...
        lprioridade +=
            fcalculaPrioridadeNome(lid, lepisodioAnime, lepisodiosAssistidos,
                                   lista, lposicaoAnimeNaLista);
      ...
      if (stream.name() == "description") {
        ...
        // Checamos se já existe um anime na lista de downloads com esse nome e
        // episódio
        if (vbaixar.contains(lnomeAnime + lepisodioAnime)) {
          // Caso o torrent existente tenha menos prioridade do que o torrent
          // atual, ele é substituido A prioridade mínima é 14. Com 14, posso ter
          // um conjunto de dois: sub, resolução e (lingua+keyword) E sempre tem
          // que não ter sido baixado
          if (vbaixar.value(lnomeAnime + lepisodioAnime).at(0).toInt() < 16 &&
              lprioridade == 16) {
            QStringList value = (QString::number(lprioridade) + ":" +
                                 QString::number(torrent.size()))
                                    .split(":");
            vbaixar.insert(lnomeAnime + lepisodioAnime, value);
          } else if (vbaixar.value(lnomeAnime + lepisodioAnime).at(0).toInt() <
                         15 &&
                     lprioridade == 15) {
            QStringList value = (QString::number(lprioridade) + ":" +
                                 QString::number(torrent.size()))
        ...
        // Caso não exista nenhum torrent na lista de downloads com o anime
        // certo, é colocado um, mesmo que Não esteja nas condições ideiais.
  ...
  for (int i = 0; i < torrent.size(); i++) {
    for (int w = 0; w < ui->listaTorrents->columnCount(); w++) {
      ...
      case 4:
        ...
        break;
      case 5:
        if (torrent[i]->vepisodioAnime.toInt() < 10 &&
            !torrent[i]->vepisodioAnime.contains("0"))
...
void janelatorrent::fpassaPonteiros(leitorlistaanimes *lleitor,
                                    janeladeconfig *lconf, arquivos *lcarq) {
  cleitor = lleitor;
  cconfig = lconf;
  carquivos = lcarq;
}

void janelatorrent::on_botaoDownload_clicked() { ////CHECAR SE É WINDOWS OU
                                                 ///LINUX NESSA PARTE
  ...
  for (int i = 0; i < torrent.size(); i++) {
    if (torrent[i]->vbaixar) {
      ...
      qdown->fdownloadTorrent(torrent[i]->vlinkTorrent,
                              torrent[i]->vnomeTorrent);
      torrent[i]->vbaixar = false;
      achouPraDownload = true;
      connect(qdown, &filedownloader::storrent, this,
              &janelatorrent::fesperaTorrent);
    }
  }
  if (achouPraDownload) {
    QProcess lprocesso;
    if (cconfig->fretornaTorrentEscolhido() == "uTorrent")
      lprocesso.setProgram(QDir::homePath() +
                           "/AppData/Roaming/uTorrent/uTorrent.exe");
    else if (cconfig->fretornaTorrentEscolhido() == "qBittorrent")
      lprocesso.setProgram(QDir::rootPath() +
...
void janelatorrent::fbaixaTorrent() {
  ...
  if (cconfig->fretornaTorrentEscolhido() == "uTorrent") {
    ...
    argumentos.append(cconfig->fretornaPastaSalvarAnimes() + "/" +
                      torrent[vlistaDownload[0]]->vnomeAnime);
    ...
    argumentos.append("--save-path=" + cconfig->fretornaPastaSalvarAnimes() +
                      "/" + torrent[vlistaDownload[0]]->vnomeAnime);
    argumentos.append(QDir::currentPath() + "/Configurações/Temp/Torrents/" +
                      torrent[vlistaDownload[0]]->vnomeTorrent + ".torrent");
  }
...
void janelatorrent::on_botaoAtualizaLista_clicked() {
  ...
  connect(lbaixaXML, &filedownloader::sxml, this,
          &janelatorrent::fpreparaLeituraArquivoTorrent);
}
...
QString janelatorrent::fchecaFiltroFansub(const QString &lid) {
  for (int i = 0; i < cconfig->vfiltrosAnimes.size(); i++) {
    for (int w = 0; w < cconfig->vfiltrosAnimes[i]->idAnimesAfetados.size();
         w++) {
      if (cconfig->vfiltrosAnimes[i]->idAnimesAfetados[w].compare(lid) == 0) {
        if (cconfig->vfiltrosAnimes[i]->filtroDeCondicao.contains(
                "fansub", Qt::CaseInsensitive))
          return cconfig->vfiltrosAnimes[i]->filtroDeCondicao + ";" +
                 cconfig->vfiltrosAnimes[i]->filtroEspecifico;
      }
    }
  }
  return nullptr;
...
void janelatorrent::on_botaoInfoAnime_clicked() {
  if (!ui->listaTorrents->selectionModel()->selectedRows().isEmpty()) {
    QModelIndexList select =
        ui->listaTorrents->selectionModel()->selectedRows();
...
int janelatorrent::fcalculaPrioridadeNome(QString ridAnime,
                                          const QString &repisodioAime,
                                          const QString &repisodiosAssistidos,
                                          const QString &rlista,
                                          int rposicaoNaLista) {
  ...
  if (!filtros.isEmpty()) {
    if (filtros.at(0).contains("not", Qt::CaseInsensitive))
      ...
      if (repisodioAime.toInt() > repisodiosAssistidos.toInt()) {
        // Checa se é o próximo episódio que deve ser assistido. Se estiver
        // vazio, o episódio não foi baixado Se não estiver, o episódio já existe
        // no computador.
        if (rposicaoNaLista != -1 || vlistaAtual.isEmpty()) {
          QString lultimoEpisodioBaixado =
              carquivos->fprocuraEpisodioEspecifico(
                  vlistaAtual[rposicaoNaLista], repisodioAime.toInt());
          // Caso não exista o episódio na pasta, a string retorna vazia.
          lultimoEpisodioBaixado = lultimoEpisodioBaixado.mid(
              lultimoEpisodioBaixado.lastIndexOf("/") + 1);
    ...
    if (rlista.compare("Watching", Qt::CaseInsensitive) == 0 &&
      ...
      vlistaAtual.clear();
    if (repisodioAime.toInt() > repisodiosAssistidos.toInt()) {
      // Checa se é o próximo episódio que deve ser assistido. Se estiver vazio,
      // o episódio não foi baixado Se não estiver, o episódio já existe no
      // computador.
      if (rposicaoNaLista != -1) {
        ...
        if (lultimoEpisodioBaixado.isEmpty()) {
          //                                qDebug() <<
          //                                vlistaAtual[lposicaoAnimeNaLista]->vid
          //                                <<
          //                                vlistaAtual[lposicaoAnimeNaLista]->vnome;
          return 10;
        }
...
int janelatorrent::fcalculaPrioridadeSub(const QString &rfansub,
                                         const QString &rid) {
  ...
  if (!filtros.isEmpty()) {
    ...
    if (rfansub.compare(cconfig->fretornaSubEscolhido(), Qt::CaseInsensitive) ==
        0)
      return 2;
  }
  return 0;
...
int janelatorrent::fcalculaPrioridadeQualidade(const QString &resolucao,
                                               const QString &id) {
  ...
  if (!filtros.isEmpty()) {
    ...
    if (cconfig->fretornaQualidadeEscolhida().compare(resolucao.toUtf8(),
...
int janelatorrent::fcalculaPrioridadeFiltros(const QString &description,
                                             const QString &rid) {
