...
// Vamos testar todas as combinações possíveis. Se for igual em alguma,
// retornamos true. se não for igual em nenhuma, é false.
bool FormataPalavras::fcomparaNomes(const QString &rnome1,
                                    const QString &rnome2) {
  ...
  if (fremoveCaracteresEspeciais(rnome1).compare(
          fremoveCaracteresEspeciais(rnome2)) == 0)
    return true;

  if (fmudaCaracteresEspeciais(rnome1).compare(
          fmudaCaracteresEspeciais(rnome2)) == 0)
    return true;

  if (fmudaNumeracaoArabePraRomana(rnome1).compare(
          fmudaNumeracaoArabePraRomana(rnome2)) == 0)
    return true;

  if (fremoveNumeracao(rnome1).compare(fremoveNumeracao(rnome2)) == 0)
    return true;

  if (fmudaNumeracaoRomanaPraArabe(rnome1).compare(
          fmudaNumeracaoRomanaPraArabe(rnome2)) == 0)
    return true;

  if (fmudaNumeracaoRomanaPraSX(rnome1).compare(
          fmudaNumeracaoRomanaPraSX(rnome2)) == 0)
    return true;

  if (fmudaNumeracaoRomanaPraSeason(rnome1).compare(
          fmudaNumeracaoRomanaPraSeason(rnome2)) == 0)
    return true;

  if (fremoveNumeracaoRomana(rnome1).compare(fremoveNumeracaoRomana(rnome2)) ==
      0)
    return true;

  if (fremoveCaracteresExtras(rnome1).compare(
          fremoveCaracteresExtras(rnome2)) == 0)
    return true;

  if (rnome1.contains("OVA") || rnome1.contains("Special") ||
      rnome2.contains("OVA") || rnome2.contains("Special")) {
...
QString FormataPalavras::fmudaNumeracaoRomanaPraArabe(QString rpalavra) {
  // Os números com mais caracteres tem que ser testados antes, ou vai ficar 1X,
  // 1V, 2I
  rpalavra = rpalavra.toLower();
